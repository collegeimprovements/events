defmodule Examples.EnhancedSchema do
  @moduledoc """
  Example showcasing all the Events.Schema enhancements.
  """
...
defmodule Examples.UsingEnhancedSchema do
  ...
  def demo do
    IO.puts("=== Events.Schema Enhanced Features Demo ===\n")
  ...
  defp demo_introspection do
    ...
    # Get all field specs
    specs = Events.Schema.Introspection.inspect_schema(EnhancedSchema)
    IO.puts("Total fields: #{length(specs)}")

    # Get required fields
    required = Events.Schema.Introspection.required_fields(EnhancedSchema)
    IO.puts("Required fields: #{inspect(required)}")

    # Check specific field
    email_spec = Events.Schema.Introspection.inspect_field(EnhancedSchema, :email)
    IO.puts("Email field spec: #{inspect(email_spec, pretty: true, limit: :infinity)}")

    # Generate JSON schema
    json_schema = Events.Schema.Introspection.to_json_schema(EnhancedSchema)
  ...
  defp demo_validation do
    ...
    errors = Events.Schema.Errors.to_simple_map(changeset)
  ...
  defp demo_error_prioritization do
    ...
    # Group by priority
    prioritized = Events.Schema.Errors.group_by_priority(changeset)
    ...
    highest = Events.Schema.Errors.highest_priority_per_field(changeset)
