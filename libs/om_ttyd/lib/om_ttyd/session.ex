defmodule OmTtyd.Session do
  ...
  ## Options
    ...
    * `:command` - Shell command (default: system shell)
    * `:writable` - Allow terminal input (default: true)
    * `:cwd` - Working directory (default: user home or "/")
    * `:manager` - Session manager module (default: OmTtyd.SessionManager)
  ...
  def start_link(opts) do
    ...
    manager = Keyword.get(opts, :manager, OmTtyd.SessionManager)
    registry = OmTtyd.SessionManager.registry_name(manager)
  ...
  """
  @spec port(String.t(), keyword()) :: {:ok, pos_integer()} | {:error, :not_found}
  def port(session_id, opts \\ []) do
    OmTtyd.SessionManager.get_session(session_id, opts)
  end

  @doc """
  Returns info about the session.
  """
  @spec info(String.t(), keyword()) :: {:ok, map()} | {:error, :not_found}
  def info(session_id, opts \\ []) do
    manager = Keyword.get(opts, :manager, OmTtyd.SessionManager)
    registry = OmTtyd.SessionManager.registry_name(manager)

    try do
      GenServer.call({:via, Registry, {registry, session_id}}, :info)
    catch
  ...
  def init(opts) do
    ...
    case OmTtyd.start_link(command, ttyd_opts) do
      {:ok, ttyd_pid} ->
  ...
  def terminate(reason, state) do
    ...
    if state.ttyd_pid && Process.alive?(state.ttyd_pid) do
      OmTtyd.stop(state.ttyd_pid, 2000)
    end
