defmodule OmTtyd.Server do
  @moduledoc """
  Supervised ttyd terminal server.

  This module wraps `OmTtyd` as a proper child spec for supervision trees.
  ...
  Or start manually:
      ...
      {:ok, pid} = OmTtyd.Server.start_link(
        command: "bash",
        port: 7681
      )
  ...
  ## Options
    ...
    * All other options are passed to `OmTtyd.start_link/2`
  ...
  def handle_call(:url, _from, state) do
    url = OmTtyd.url(state.ttyd_pid)
    {:reply, url, state}
  end

  def handle_call(:port, _from, state) do
    port = OmTtyd.port(state.ttyd_pid)
    {:reply, port, state}
  end

  def handle_call(:info, _from, state) do
    info = OmTtyd.info(state.ttyd_pid)
    {:reply, info, state}
  end

  def handle_call(:alive?, _from, state) do
    alive = OmTtyd.alive?(state.ttyd_pid)
    {:reply, alive, state}
  end
