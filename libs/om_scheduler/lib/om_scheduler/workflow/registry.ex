defmodule OmScheduler.Workflow.Registry do
  @moduledoc """
  ...
  def init(opts) do
    ...
    # Use unique table names if a custom registry name is provided (for testing)
    name = Keyword.get(opts, :name, __MODULE__)
    workflows_table_name = :"#{name}_workflows"
    executions_table_name = :"#{name}_executions"

    workflows_table = :ets.new(workflows_table_name, [:set, :protected, :named_table])
    executions_table = :ets.new(executions_table_name, [:set, :protected, :named_table])
