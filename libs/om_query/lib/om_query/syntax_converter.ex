defmodule OmQuery.SyntaxConverter do
  @moduledoc false
  ...
  defp operation_to_pipeline({:filter, {field, op, value, opts}}) do
    if opts == [] do
      "|> OmQuery.filter(#{inspect(field)}, #{inspect(op)}, #{format_value(value)})"
    else
      "|> OmQuery.filter(#{inspect(field)}, #{inspect(op)}, #{format_value(value)}, #{inspect(opts)})"
    end
  end

  defp operation_to_pipeline({:order, {field, direction, opts}}) do
    if opts == [] do
      "|> OmQuery.order(#{inspect(field)}, #{inspect(direction)})"
    else
      "|> OmQuery.order(#{inspect(field)}, #{inspect(direction)}, #{inspect(opts)})"
    end
  end
  ...
  defp operation_to_pipeline({:offset, value}) do
    "|> OmQuery.offset(#{value})"
  end

  defp operation_to_pipeline({:paginate, {type, opts}}) do
    "|> OmQuery.paginate(#{inspect(type)}, #{format_keyword_list(opts)})"
  end

  defp operation_to_pipeline({:join, {assoc, type, opts}}) do
    if opts == [] do
      "|> OmQuery.join(#{inspect(assoc)}, #{inspect(type)})"
    else
      "|> OmQuery.join(#{inspect(assoc)}, #{inspect(type)}, #{inspect(opts)})"
    end
  end

  defp operation_to_pipeline({:preload, {assoc, nil}}) do
    "|> OmQuery.preload(#{inspect(assoc)})"
  end

  defp operation_to_pipeline({:preload, {assoc, fun}}) when is_function(fun) do
    "|> OmQuery.preload(#{inspect(assoc)}, fn token -> token end)"
  end
  ...
  defp add_spec_pagination_pipeline(lines, {:paginate, type, config, _opts}) do
    ...
    lines ++ ["|> OmQuery.paginate(#{inspect(type)}, #{opts_str})"]
  end
