defmodule OmQuery.TestHelpers do
  @moduledoc """
  Test utilities for the OmQuery system.
  ...
  Add to your test case:
      ...
      use OmQuery.TestHelpers

  Or import specific functions:

      import OmQuery.TestHelpers
  ...
  defmacro assert_has_operation(token, type) do
    quote do
      ...
      unless OmQuery.TestHelpers.has_operation?(token, type) do
        ops = OmQuery.TestHelpers.get_operations(token)
  ...
  defmacro refute_has_operation(token, type) do
    quote do
      ...
      if OmQuery.TestHelpers.has_operation?(token, type) do
        raise ExUnit.AssertionError,
  ...
  defmacro assert_filter_includes(token, field, op, value) do
    quote do
      ...
      filters = OmQuery.TestHelpers.get_operations_by_type(token, :filter)
  ...
  defmacro assert_sql_contains(token, substring) do
    quote do
      ...
      case OmQuery.TestHelpers.to_sql(token) do
        {:ok, sql} ->
  ...
  defmacro assert_pagination_type(token, expected_type) do
    quote do
      ...
      paginations = OmQuery.TestHelpers.get_operations_by_type(token, :paginate)
  ...
  defmacro assert_order_includes(token, field, direction) do
    quote do
      ...
      orders = OmQuery.TestHelpers.get_operations_by_type(token, :order)
  ...
  defmacro assert_join_includes(token, assoc, join_type \\ :inner) do
    quote do
      ...
      joins = OmQuery.TestHelpers.get_operations_by_type(token, :join)
  ...
  defmacro assert_operation_count(token, type, expected_count) do
    quote do
      ...
      ops = OmQuery.TestHelpers.get_operations_by_type(token, type)
  ...
  ## Examples
      ...
      token1 = User |> OmQuery.new() |> OmQuery.filter(:status, :eq, "active")
      token2 = User |> OmQuery.new() |> OmQuery.filter(:status, :eq, "active")
  ...
  defmacro assert_tokens_equivalent(token1, token2) do
    quote do
      ...
      unless OmQuery.TestHelpers.tokens_equivalent?(t1, t2) do
        raise ExUnit.AssertionError,
          message: """
