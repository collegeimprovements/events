defmodule OmQuery.Debug do
  ...
  |> OmQuery.new()
  |> OmQuery.filter(:status, :eq, "active")
  |> OmQuery.debug(:raw_sql, label: "After filter", pretty: true)
  |> OmQuery.execute()

  # Multiple formats at once
  token |> OmQuery.debug([:raw_sql, :pipeline])
  ...
  ## Examples
      ...
      # Specify format
      OmQuery.debug(token, :pipeline)
      OmQuery.debug(token, :dsl)
      OmQuery.debug(token, [:raw_sql, :token])

      # With options
      OmQuery.debug(token, :raw_sql, label: "Product Query", color: :green)
  """
  ...
  defp faceted_to_token(%FacetedSearch{
    ...
    token =
      case search_config do
        {term, fields, opts} when is_binary(term) and term != "" ->
          OmQuery.search(token, term, fields, opts)
    ...
    # Apply filters
    token = OmQuery.filter_by(token, filters)

    # Apply ordering
    token =
      Enum.reduce(ordering, token, fn {field, direction}, acc ->
        OmQuery.order(acc, field, direction)
      end)

    # Apply pagination
    token =
      case pagination do
        {type, opts} -> OmQuery.paginate(token, type, opts)
        _ -> token
      end
    ...
    token =
      case preloads do
        [] -> token
        preloads -> OmQuery.preload(token, preloads)
      end

    # Apply select
    case select_fields do
      nil -> token
      fields -> OmQuery.select(token, fields)
    end
  end
  ...
  defp find_caller_location(stack) do
    ...
    |> Enum.find(fn {mod, _fun, _arity, _loc} ->
      ...
      not String.starts_with?(mod_string, "OmQuery.Debug") and
        not String.starts_with?(mod_string, "Elixir.OmQuery.Debug") and
