defmodule OmQuery.Demo do
  ...
  #
  # Quick Start:
  #   iex> OmQuery.Demo.token_composition()
  #   iex> OmQuery.Demo.token_inspection()
  #   iex> OmQuery.Demo.result_structure()
  ...
  def filter_validation do
    ...
    token =
      token
      |> OmQuery.filter(:price, :gte, 10.00)
      |> OmQuery.filter(:category, :in, ["electronics", "gadgets"])
      |> OmQuery.filter(:name, :ilike, "%laptop%")
  ...
  def query_building do
    ...
    token =
      OmQuery.new(:orders)
      |> OmQuery.filter(:status, :in, ["pending", "processing"])
      |> OmQuery.filter(:total, :gte, 100.00)
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(50)
