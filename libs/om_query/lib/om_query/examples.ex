defmodule OmQuery.Examples do
  @moduledoc false
  # Documentation examples module - not part of public API.
  #
  # Contains documentation examples using placeholder schemas (User, Post, etc.)
  # that don't exist in this application. These functions are for reference only.

  # Suppress warnings for undefined schemas (these are documentation examples)
  @compile {:no_warn_undefined,
            [
              User,
              Post,
              Comment,
              Product,
              Order,
              Sale,
              Report,
              Category,
              Brand,
              Activity,
              Project,
              Subscription,
              Invoice
            ]}

  import OmQuery.DSL
  alias OmQuery

  # Get configured repo or raise
  defp repo do
    Application.get_env(:om_query, :repo) ||
      raise "OmQuery.Examples requires :repo to be configured in :om_query application"
  end

  ## 1. Basic Queries

  @doc "Simple query with filters"
  def basic_query do
    query User do
      filter(:status, :eq, "active")
      filter(:age, :gte, 18)
      order(:name, :asc)
      limit(10)
    end
    |> OmQuery.execute()
  end

  @doc "Pipeline style"
  def pipeline_query do
    User
    |> OmQuery.new()
    |> OmQuery.filter(:status, :eq, "active")
    |> OmQuery.order(:created_at, :desc)
    |> OmQuery.limit(20)
    |> OmQuery.execute()
  end

  ## 2. Filtering

  @doc "All filter operators"
  def filter_operators do
    query Product do
      # Equality
      filter(:status, :eq, "active")
      filter(:category, :neq, "archived")

      # Comparisons
      filter(:price, :gte, 10.00)
      filter(:stock, :gt, 0)

      # List membership
      filter(:category, :in, ["electronics", "gadgets"])
      filter(:status, :not_in, ["deleted", "archived"])

      # Pattern matching
      filter(:name, :ilike, "%widget%")

      # Null checks
      filter(:deleted_at, :is_nil, nil)

      # Range
      filter(:price, :between, {10.00, 100.00})

      # JSONB
      filter(:metadata, :jsonb_contains, %{featured: true})
      filter(:tags, :jsonb_has_key, "priority")
    end
    |> OmQuery.execute()
  end

  @doc "Filter options"
  def filter_with_options do
    query User do
      # Case insensitive comparison
      filter(:email, :eq, "john@example.com", case_insensitive: true)

      # Filter on joined table
      join(:posts, :inner, as: :user_posts)
      filter(:published, :eq, true, binding: :user_posts)
    end
    |> OmQuery.execute()
  end

  ## 3. Pagination

  @doc "Offset pagination"
  def offset_pagination(page \\ 1, per_page \\ 20) do
    offset = (page - 1) * per_page

    query Post do
      filter(:published, :eq, true)
      order(:published_at, :desc)
      paginate(:offset, limit: per_page, offset: offset)
    end
    |> OmQuery.execute(include_total_count: true)
  end

  @doc "Cursor pagination"
  def cursor_pagination(after_cursor \\ nil) do
    opts =
      [cursor_fields: [:published_at, :id], limit: 20]
      |> then(fn opts ->
        if after_cursor, do: Keyword.put(opts, :after, after_cursor), else: opts
      end)

    query Post do
      filter(:published, :eq, true)
      order(:published_at, :desc)
      order(:id, :desc)
      paginate(:cursor, opts)
    end
    |> OmQuery.execute()
  end

  @doc "Get next page from cursor pagination result"
  def get_next_page(result) do
    if result.pagination.has_more do
      cursor_pagination(result.pagination.end_cursor)
    else
      {:error, :no_more_pages}
    end
  end

  ## 4. Joins and Preloads

  @doc "Association preloads"
  def simple_preloads do
    query User do
      filter(:status, :eq, "active")
      preload([:posts, :comments, :profile])
    end
    |> OmQuery.execute()
  end

  @doc "Nested preloads with filters"
  def nested_preloads do
    query User do
      filter(:status, :eq, "active")

      # Preload published posts with their comments
      preload :posts do
        filter(:published, :eq, true)
        order(:published_at, :desc)
        limit(5)

        # Nested preload
        preload :comments do
          filter(:approved, :eq, true)
          order(:created_at, :desc)
          limit(3)
        end
      end

      # Preload recent comments
      preload :comments do
        filter(:created_at, :gte, ~N[2024-01-01 00:00:00])
        order(:created_at, :desc)
      end
    end
    |> OmQuery.execute()
  end

  @doc "Joins with custom conditions"
  def custom_joins do
    query User do
      filter(:status, :eq, "active")

      # Association join
      join(:posts, :left, as: :user_posts)

      # Filter on joined table
      filter(:published, :eq, true, binding: :user_posts)

      select(%{
        user_name: :name,
        user_email: :email
      })
    end
    |> OmQuery.execute()
  end

  ## 5. Aggregations

  @doc "Group by and having"
  def aggregations do
    query Order do
      filter(:created_at, :gte, ~D[2024-01-01])

      group_by([:status])

      having(count: {:gte, 10})

      select([:id, :status, :created_at])
    end
    |> OmQuery.execute()
  end

  @doc "Complex aggregation with joins"
  def complex_aggregation do
    query User do
      join(:orders, :left)

      group_by([:id, :name])

      having(count: {:gte, 5})

      select([:id, :name, :email])

      order(:name, :desc)
      limit(10)
    end
    |> OmQuery.execute()
  end

  ## 6. CTEs and Subqueries

  @doc "Common Table Expression"
  def with_cte do
    # Define CTE for active users
    active_users_cte =
      query User do
        filter(:status, :eq, "active")
        filter(:last_login, :gte, ~N[2024-01-01 00:00:00])
        select([:id, :name, :email])
      end

    # Use CTE in main query
    query Order do
      with_cte(:active_users, active_users_cte)

      # Filter orders (CTE usage would require additional join support)
      filter(:status, :eq, "completed")

      order(:created_at, :desc)
    end
    |> OmQuery.execute()
  end

  @doc "Multiple CTEs"
  def multiple_ctes do
    recent_users =
      query User do
        filter(:created_at, :gte, ~N[2024-01-01 00:00:00])
      end

    active_orders =
      query Order do
        filter(:status, :in, ["pending", "processing"])
      end

    query Report do
      with_cte(:recent_users, recent_users)
      with_cte(:active_orders, active_orders)

      # Use both CTEs in analysis
    end
    |> OmQuery.execute()
  end

  ## 7. Window Functions

  @doc "Running totals with window functions"
  def running_totals do
    query Sale do
      filter(:created_at, :gte, ~D[2024-01-01])

      window(:running_total,
        partition_by: :product_id,
        order_by: [asc: :sale_date]
      )

      select(%{
        sale_id: :id,
        product_id: :product_id,
        amount: :amount,
        running_total: {:window, :sum, :amount, :running_total}
      })

      order(:sale_date, :asc)
    end
    |> OmQuery.execute()
  end

  @doc "Ranking with window functions"
  def product_ranking do
    query Product do
      filter(:active, :eq, true)

      window(:category_rank,
        partition_by: :category_id,
        order_by: [desc: :sales_count]
      )

      select(%{
        product_name: :name,
        category_id: :category_id,
        sales_count: :sales_count,
        rank: {:window, :rank, :sales_count, :category_rank}
      })
    end
    |> OmQuery.execute()
  end

  ## 8. Transactions

  @doc "Execute query in transaction"
  def transactional_query do
    OmQuery.transaction(fn ->
      # Update user
      user_token =
        User
        |> OmQuery.new()
        |> OmQuery.filter(:id, :eq, 123)

      user_result = OmQuery.execute(user_token)

      # Create order
      # ... more operations

      {:ok, user_result}
    end)
  end

  @doc "Multi-step transaction with Ecto.Multi"
  def multi_transaction do
    alias OmQuery.Multi, as: QM

    user_query =
      User
      |> OmQuery.new()
      |> OmQuery.filter(:id, :eq, 123)

    posts_query =
      Post
      |> OmQuery.new()
      |> OmQuery.filter(:user_id, :eq, 123)

    Ecto.Multi.new()
    |> QM.query(:user, user_query)
    |> QM.query(:posts, posts_query)
    |> Ecto.Multi.run(:summary, fn _repo, %{user: user, posts: posts} ->
      {:ok,
       %{
         user_name: List.first(user.data).name,
         post_count: length(posts.data)
       }}
    end)
    |> repo().transaction()
  end

  ## 9. Batch Operations

  @doc "Execute multiple queries in parallel"
  def batch_queries do
    users_token =
      User
      |> OmQuery.new()
      |> OmQuery.filter(:status, :eq, "active")
      |> OmQuery.limit(100)

    posts_token =
      Post
      |> OmQuery.new()
      |> OmQuery.filter(:published, :eq, true)
      |> OmQuery.limit(100)

    comments_token =
      Comment
      |> OmQuery.new()
      |> OmQuery.filter(:approved, :eq, true)
      |> OmQuery.limit(100)

    [users_result, posts_result, comments_result] =
      OmQuery.batch([users_token, posts_token, comments_token])

    %{
      users: length(users_result.data),
      posts: length(posts_result.data),
      comments: length(comments_result.data)
    }
  end

  ## 10. Advanced Patterns

  @doc "Dynamic filter building"
  def dynamic_filters(filters) do
    base_token = OmQuery.new(Product)

    final_token =
      Enum.reduce(filters, base_token, fn
        {:category, category}, token ->
          OmQuery.filter(token, :category, :eq, category)

        {:min_price, price}, token ->
          OmQuery.filter(token, :price, :gte, price)

        {:max_price, price}, token ->
          OmQuery.filter(token, :price, :lte, price)

        {:search, term}, token ->
          OmQuery.filter(token, :name, :ilike, "%#{term}%")

        _, token ->
          token
      end)

    OmQuery.execute(final_token)
  end

  @doc "Streaming large result sets"
  def stream_results do
    token =
      User
      |> OmQuery.new()
      |> OmQuery.filter(:status, :eq, "active")
      |> OmQuery.order(:id, :asc)

    token
    |> OmQuery.stream(max_rows: 1000)
    |> Enum.each(fn user ->
      # Process each user
      IO.inspect(user.id)
    end)
  end

  @doc "Raw SQL with named placeholders"
  def raw_sql_query do
    query User do
      raw_where("age BETWEEN :min_age AND :max_age", %{
        min_age: 18,
        max_age: 65
      })

      raw_where("created_at >= :start_date", %{
        start_date: ~N[2024-01-01 00:00:00]
      })

      order(:created_at, :desc)
    end
    |> OmQuery.execute()
  end

  @doc "Build query without executing"
  def build_only do
    token =
      User
      |> OmQuery.new()
      |> OmQuery.filter(:status, :eq, "active")
      |> OmQuery.order(:name, :asc)

    # Get the Ecto.Query without executing
    ecto_query = OmQuery.build(token)

    # Can inspect or modify the query
    IO.inspect(ecto_query)

    # Execute manually
    repo().all(ecto_query)
  end

  @doc "Custom result processing"
  def custom_processing do
    result =
      query User do
        filter(:status, :eq, "active")
        order(:created_at, :desc)
        limit(100)
      end
      |> OmQuery.execute()

    case result do
      %{data: data, pagination: pagination} ->
        %{
          users: data,
          count: length(data),
          has_more: pagination.has_more,
          query_time_ms: result.metadata.query_time_μs / 1000
        }
    end
  end

  # ============================================================================
  # 11. ADVANCED PATTERNS - Complex Real-World Examples
  # ============================================================================

  @doc """
  ## Social Feed with Nested Preloads

  Load a user's feed with posts, their authors, comments with commenter profiles,
  and like counts - all with proper filtering and pagination.

  ### The Complex Problem
  - Get published posts from followed users
  - For each post: author with avatar, top 3 comments with commenters
  - Paginate with cursor (stable for real-time feeds)
  - Filter out blocked users' content

  ### Without Query DSL (verbose Ecto)
  ```elixir
  # Would require ~50+ lines of nested preload queries, dynamic joins,
  # manual cursor encoding, and complex where clauses
  ```

  ### With Query DSL (clean & composable)
  """
  def social_feed(_user_id, cursor \\ nil) do
    # Build the base query with all complexity hidden
    Post
    |> OmQuery.new()
    |> OmQuery.filter(:published, :eq, true)
    |> OmQuery.filter(:visibility, :in, ["public", "friends"])
    |> OmQuery.order(:published_at, :desc)
    # Tiebreaker for stable cursor
    |> OmQuery.order(:id, :asc)
    # Conditional cursor pagination
    |> OmQuery.then_if(cursor, fn token, c ->
      OmQuery.paginate(token, :cursor, limit: 20, after: c)
    end)
    |> OmQuery.if_true(is_nil(cursor), fn token ->
      OmQuery.paginate(token, :cursor, limit: 20)
    end)
    # Author with profile (nested preload)
    |> OmQuery.preload(:author, fn author_token ->
      author_token
      |> OmQuery.preload(:profile)
      |> OmQuery.select([:id, :username, :avatar_url])
    end)
    # Top comments with their authors
    |> OmQuery.preload(:comments, fn comments_token ->
      comments_token
      |> OmQuery.filter(:approved, :eq, true)
      |> OmQuery.filter(:deleted_at, :is_nil, true)
      |> OmQuery.order(:likes_count, :desc)
      |> OmQuery.order(:created_at, :asc)
      |> OmQuery.limit(3)
      |> OmQuery.preload(:author, fn author ->
        author |> OmQuery.select([:id, :username, :avatar_url])
      end)
    end)
    |> OmQuery.execute()
  end

  @doc """
  ## E-Commerce Product Listing with Faceted Filters

  Complex product search with:
  - Dynamic filters (category, price range, attributes)
  - Inventory join to check stock
  - Seller rating from reviews
  - Multiple sort options
  - Cursor pagination

  ### The Pattern
  Combine `filter_by`, `then_if`, joins, and fragments for maximum flexibility.
  """
  def product_listing(params) do
    Product
    |> OmQuery.new()
    |> OmQuery.filter(:active, :eq, true)
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Dynamic equality filters from params
    |> OmQuery.filter_by(Map.take(params, [:category_id, :brand_id, :seller_id]))
    # Conditional range filters
    |> OmQuery.then_if(params[:min_price], fn token, price ->
      OmQuery.filter(token, :price, :gte, price)
    end)
    |> OmQuery.then_if(params[:max_price], fn token, price ->
      OmQuery.filter(token, :price, :lte, price)
    end)
    # Search in name and description
    |> OmQuery.then_if(params[:search], fn token, search ->
      OmQuery.where_any(token, [
        {:name, :ilike, "%#{search}%"},
        {:description, :ilike, "%#{search}%"},
        {:sku, :eq, search}
      ])
    end)
    # JSONB attribute filters (e.g., color, size)
    |> OmQuery.then_if(params[:attributes], fn token, attrs ->
      OmQuery.filter(token, :attributes, :jsonb_contains, attrs)
    end)
    # Join inventory for stock check
    |> OmQuery.join(:inventory, :left, as: :inv)
    |> OmQuery.if_true(params[:in_stock_only], fn token ->
      OmQuery.filter(token, :quantity, :gt, 0, binding: :inv)
    end)
    # Dynamic sorting
    |> apply_product_sort(params[:sort])
    # Cursor pagination
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 24)
    # Preload related data
    |> OmQuery.preload([:category, :brand, :primary_image])
    |> OmQuery.preload(:reviews, fn reviews ->
      reviews
      |> OmQuery.filter(:approved, :eq, true)
      |> OmQuery.order(:helpful_count, :desc)
      |> OmQuery.limit(3)
    end)
    |> OmQuery.execute()
  end

  defp apply_product_sort(token, "price_asc"), do: OmQuery.orders(token, asc: :price, asc: :id)
  defp apply_product_sort(token, "price_desc"), do: OmQuery.orders(token, desc: :price, asc: :id)
  defp apply_product_sort(token, "newest"), do: OmQuery.orders(token, desc: :created_at, asc: :id)
  defp apply_product_sort(token, "popular"), do: OmQuery.orders(token, desc: :sales_count, asc: :id)
  defp apply_product_sort(token, "rating"), do: OmQuery.orders(token, desc: :avg_rating, asc: :id)
  defp apply_product_sort(token, _), do: OmQuery.orders(token, desc: :featured_score, asc: :id)

  @doc """
  ## Admin Dashboard with Multiple Aggregations

  Complex admin view showing:
  - Users with their order counts and total spent
  - Filtered by date range and status
  - With nested organization data
  - Paginated with total count

  ### The Pattern
  Joins + Group By + Having + Nested Preloads + Offset Pagination (for admin UI)
  """
  def admin_user_dashboard(params) do
    User
    |> OmQuery.new()
    # Base filters
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    |> OmQuery.filter_by(Map.take(params, [:status, :role, :organization_id]))
    # Date range filter
    |> OmQuery.then_if(params[:created_after], fn token, date ->
      OmQuery.filter(token, :created_at, :gte, date)
    end)
    |> OmQuery.then_if(params[:created_before], fn token, date ->
      OmQuery.filter(token, :created_at, :lte, date)
    end)
    # Search across multiple fields
    |> OmQuery.then_if(params[:search], fn token, search ->
      OmQuery.where_any(token, [
        {:name, :ilike, "%#{search}%"},
        {:email, :ilike, "%#{search}%"},
        {:phone, :ilike, "%#{search}%"}
      ])
    end)
    # Join orders for aggregation
    |> OmQuery.join(:orders, :left, as: :orders)
    |> OmQuery.group_by(:id)
    # Filter by order count
    |> OmQuery.then_if(params[:min_orders], fn token, min ->
      OmQuery.having(token, count: {:gte, min})
    end)
    # Sorting
    |> OmQuery.orders(parse_admin_sort(params[:sort]))
    # Offset pagination with total count (better for admin tables)
    |> OmQuery.paginate(:offset,
      limit: params[:per_page] || 25,
      offset: ((params[:page] || 1) - 1) * (params[:per_page] || 25)
    )
    # Rich preloads for admin view
    |> OmQuery.preload(:organization)
    |> OmQuery.preload(:profile)
    |> OmQuery.preload(:last_order, fn order ->
      order
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(1)
    end)
    |> OmQuery.execute(include_total_count: true)
  end

  defp parse_admin_sort("-" <> field), do: [{:desc, String.to_existing_atom(field)}]
  defp parse_admin_sort("+" <> field), do: [{:asc, String.to_existing_atom(field)}]
  defp parse_admin_sort(field) when is_binary(field), do: [{:asc, String.to_existing_atom(field)}]
  defp parse_admin_sort(_), do: [{:desc, :created_at}, {:asc, :id}]

  @doc """
  ## Hierarchical Data with Recursive CTE

  Load an organization tree with:
  - All descendants of a parent org
  - Employee counts at each level
  - Budget rollups

  ### The Pattern
  Recursive CTE + Aggregation + Nested Structure
  """
  def organization_tree(root_org_id) do
    import Ecto.Query

    # Base case: the root organization
    base =
      from(o in "organizations",
        where: o.id == ^root_org_id,
        select: %{
          id: o.id,
          name: o.name,
          parent_id: o.parent_id,
          depth: 0
        }
      )

    # Recursive case: children of organizations in the tree
    recursive =
      from(o in "organizations",
        join: tree in "org_tree",
        on: o.parent_id == tree.id,
        select: %{
          id: o.id,
          name: o.name,
          parent_id: o.parent_id,
          depth: tree.depth + 1
        }
      )

    cte_query = union_all(base, ^recursive)

    # Main query using the CTE
    from(o in "org_tree")
    |> OmQuery.new()
    |> OmQuery.with_cte(:org_tree, cte_query, recursive: true)
    |> OmQuery.order(:depth, :asc)
    |> OmQuery.order(:name, :asc)
    # No pagination for tree data
    |> OmQuery.execute(unsafe: true)
  end

  @doc """
  ## Real-Time Notifications with Complex Filters

  Load notifications with:
  - Polymorphic source resolution (post, comment, user, etc.)
  - Read/unread filtering
  - Grouped by date
  - With actor information

  ### The Pattern
  OR groups + Polymorphic preloads + Conditional includes
  """
  def user_notifications(user_id, params \\ %{}) do
    Notification
    |> OmQuery.new()
    |> OmQuery.filter(:user_id, :eq, user_id)
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Filter by read status
    |> OmQuery.then_if(params[:unread_only], fn token, true ->
      OmQuery.filter(token, :read_at, :is_nil, true)
    end)
    # Filter by notification types
    |> OmQuery.then_if(params[:types], fn token, types ->
      OmQuery.filter(token, :type, :in, types)
    end)
    # Filter by date range
    |> OmQuery.then_if(params[:since], fn token, since ->
      OmQuery.filter(token, :created_at, :gte, since)
    end)
    # Order by most recent
    |> OmQuery.order(:created_at, :desc)
    |> OmQuery.order(:id, :desc)
    # Cursor pagination
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 20)
    # Preload actor (who triggered the notification)
    |> OmQuery.preload(:actor, fn actor ->
      actor |> OmQuery.select([:id, :username, :avatar_url])
    end)
    # Preload the source based on type (polymorphic)
    |> OmQuery.preload(:source)
    |> OmQuery.execute()
  end

  @doc """
  ## Analytics Query with Window Functions

  Calculate running totals, rankings, and percentiles
  for sales data.

  ### The Pattern
  Window functions + Partitioning + Frame specification
  """
  def sales_analytics(params) do
    Sale
    |> OmQuery.new()
    |> OmQuery.filter(:status, :eq, "completed")
    |> OmQuery.then_if(params[:start_date], fn token, date ->
      OmQuery.filter(token, :completed_at, :gte, date)
    end)
    |> OmQuery.then_if(params[:end_date], fn token, date ->
      OmQuery.filter(token, :completed_at, :lte, date)
    end)
    |> OmQuery.then_if(params[:product_ids], fn token, ids ->
      OmQuery.filter(token, :product_id, :in, ids)
    end)
    # Define windows for analytics
    |> OmQuery.window(:running_total,
      partition_by: :product_id,
      order_by: [asc: :completed_at],
      frame: {:rows, :unbounded_preceding, :current_row}
    )
    |> OmQuery.window(:daily_rank,
      partition_by: :product_id,
      order_by: [desc: :amount]
    )
    |> OmQuery.order(:completed_at, :asc)
    |> OmQuery.paginate(:offset, limit: params[:limit] || 100)
    |> OmQuery.execute()
  end

  @doc """
  ## Batch Processing with Streaming

  Process large datasets efficiently without loading all into memory.

  ### The Pattern
  Streaming + Transaction + Chunked processing
  """
  def process_expired_subscriptions do
    OmQuery.transaction(fn ->
      Subscription
      |> OmQuery.new()
      |> OmQuery.filter(:status, :eq, "active")
      |> OmQuery.filter(:expires_at, :lt, DateTime.utc_now())
      # Allow parallel workers
      |> OmQuery.lock(:update_skip_locked)
      |> OmQuery.order(:expires_at, :asc)
      |> OmQuery.stream(max_rows: 100)
      |> Stream.each(fn subscription ->
        # Process each subscription
        expire_subscription(subscription)
      end)
      |> Stream.run()

      :ok
    end)
  end

  defp expire_subscription(_subscription), do: :ok

  # ============================================================================
  # 12. FRAGMENT COMPOSITION PATTERNS
  # ============================================================================

  # ---------------------------------------------------------------------------
  # Reusable Query Fragments
  #
  # Define common query patterns once, compose them anywhere.
  # Function-based fragments for maximum reusability.
  # ---------------------------------------------------------------------------

  defmodule Fragments do
    @moduledoc false
    alias OmQuery
    alias OmQuery.Token

    # Soft delete scope
    def not_deleted do
      Token.new(:nested)
      |> OmQuery.filter(:deleted_at, :is_nil, true)
    end

    # Active status scope
    def active do
      Token.new(:nested)
      |> OmQuery.filter(:status, :eq, "active")
    end

    # Published content scope
    def published do
      Token.new(:nested)
      |> OmQuery.filter(:published, :eq, true)
      |> OmQuery.filter(:published_at, :lte, DateTime.utc_now())
    end

    # Recent items (last N days)
    def recent(days \\ 7) do
      cutoff = DateTime.add(DateTime.utc_now(), -days * 24 * 60 * 60, :second)

      Token.new(:nested)
      |> OmQuery.filter(:created_at, :gte, cutoff)
    end

    # Standard cursor pagination
    def paginated(cursor \\ nil, limit \\ 20) do
      token =
        Token.new(:nested)
        |> OmQuery.order(:created_at, :desc)
        |> OmQuery.order(:id, :asc)

      if cursor do
        OmQuery.paginate(token, :cursor, limit: limit, after: cursor)
      else
        OmQuery.paginate(token, :cursor, limit: limit)
      end
    end

    # Visible to user (public or owned)
    def visible_to(user_id) do
      Token.new(:nested)
      |> OmQuery.where_any([
        {:visibility, :eq, "public"},
        {:user_id, :eq, user_id}
      ])
    end
  end

  @doc """
  ## Using Composed Fragments

  Combine multiple fragments for clean, readable queries.
  """
  def composed_query_example(user_id, cursor) do
    Post
    |> OmQuery.new()
    |> OmQuery.include(Fragments.not_deleted())
    |> OmQuery.include(Fragments.published())
    |> OmQuery.include(Fragments.visible_to(user_id))
    |> OmQuery.include(Fragments.recent(30))
    |> OmQuery.include(Fragments.paginated(cursor, 20))
    |> OmQuery.preload([:author, :category])
    |> OmQuery.execute()
  end

  @doc """
  ## Conditional Fragment Composition

  Apply fragments based on runtime conditions.
  """
  def conditional_fragments_example(params, current_user) do
    Post
    |> OmQuery.new()
    |> OmQuery.include(Fragments.not_deleted())
    # Only show published unless admin
    |> OmQuery.include_if(!current_user.admin?, Fragments.published())
    # Apply visibility rules unless admin
    |> OmQuery.include_if(!current_user.admin?, Fragments.visible_to(current_user.id))
    # Only recent if requested
    |> OmQuery.include_if(params[:recent_only], Fragments.recent(params[:days] || 7))
    |> OmQuery.include(Fragments.paginated(params[:cursor]))
    |> OmQuery.execute()
  end

  # ============================================================================
  # 13. SIDE-BY-SIDE: MACRO DSL vs PIPELINE
  # ============================================================================
  #
  # Every advanced pattern shown in both styles so you can choose what fits.
  # Both produce identical queries - pick based on preference.
  #

  @doc """
  ## Blog Post Feed - Both Syntaxes

  Load published blog posts with author info and comment previews.
  """
  def blog_feed_pipeline(cursor \\ nil) do
    # PIPELINE STYLE - Explicit, composable, easy to conditionally modify
    Post
    |> OmQuery.new()
    |> OmQuery.filter(:published, :eq, true)
    |> OmQuery.filter(:published_at, :lte, DateTime.utc_now())
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    |> OmQuery.order(:published_at, :desc)
    |> OmQuery.order(:id, :asc)
    |> OmQuery.then_if(cursor, fn token, c ->
      OmQuery.paginate(token, :cursor, limit: 10, after: c)
    end)
    |> OmQuery.if_true(is_nil(cursor), fn token ->
      OmQuery.paginate(token, :cursor, limit: 10)
    end)
    |> OmQuery.preload(:author, fn a ->
      a |> OmQuery.select([:id, :name, :avatar_url])
    end)
    |> OmQuery.preload(:comments, fn c ->
      c
      |> OmQuery.filter(:approved, :eq, true)
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(3)
    end)
    |> OmQuery.preload(:tags)
    |> OmQuery.execute()
  end

  def blog_feed_dsl(cursor \\ nil) do
    # MACRO DSL STYLE - Concise, declarative, SQL-like readability
    import OmQuery.DSL

    opts = if cursor, do: [limit: 10, after: cursor], else: [limit: 10]

    query Post do
      filter(:published, :eq, true)
      filter(:published_at, :lte, DateTime.utc_now())
      filter(:deleted_at, :is_nil, true)

      order(:published_at, :desc)
      order(:id, :asc)

      paginate(:cursor, opts)

      preload :author do
        select([:id, :name, :avatar_url])
      end

      preload :comments do
        filter(:approved, :eq, true)
        order(:created_at, :desc)
        limit(3)
      end

      preload([:tags])
    end
    |> OmQuery.execute()
  end

  @doc """
  ## User Search with Multiple Criteria - Both Syntaxes

  Search users by name/email with role filtering and organization scope.
  """
  def user_search_pipeline(params) do
    # PIPELINE STYLE
    User
    |> OmQuery.new()
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    |> OmQuery.filter(:status, :eq, "active")
    # Search across multiple fields (OR)
    |> OmQuery.then_if(params[:q], fn token, search ->
      OmQuery.where_any(token, [
        {:name, :ilike, "%#{search}%"},
        {:email, :ilike, "%#{search}%"},
        {:username, :ilike, "%#{search}%"}
      ])
    end)
    # Filter by role
    |> OmQuery.then_if(params[:role], fn token, role ->
      OmQuery.filter(token, :role, :eq, role)
    end)
    # Filter by roles (multiple)
    |> OmQuery.then_if(params[:roles], fn token, roles ->
      OmQuery.filter(token, :role, :in, roles)
    end)
    # Organization scope
    |> OmQuery.then_if(params[:organization_id], fn token, org_id ->
      OmQuery.filter(token, :organization_id, :eq, org_id)
    end)
    # Date range
    |> OmQuery.then_if(params[:joined_after], fn token, date ->
      OmQuery.filter(token, :created_at, :gte, date)
    end)
    |> OmQuery.then_if(params[:joined_before], fn token, date ->
      OmQuery.filter(token, :created_at, :lte, date)
    end)
    # Sorting (default: name asc)
    |> apply_user_sort(params[:sort] || "name")
    # Pagination
    |> OmQuery.paginate(:offset,
      limit: params[:per_page] || 20,
      offset: ((params[:page] || 1) - 1) * (params[:per_page] || 20)
    )
    |> OmQuery.preload([:organization, :profile])
    |> OmQuery.execute(include_total_count: true)
  end

  def user_search_dsl(params) do
    # MACRO DSL STYLE - Note: conditionals require falling back to pipeline
    import OmQuery.DSL

    base =
      query User do
        filter(:deleted_at, :is_nil, true)
        filter(:status, :eq, "active")
      end

    # For complex conditionals, combine DSL base with pipeline operations
    base
    |> OmQuery.then_if(params[:q], fn token, search ->
      OmQuery.where_any(token, [
        {:name, :ilike, "%#{search}%"},
        {:email, :ilike, "%#{search}%"},
        {:username, :ilike, "%#{search}%"}
      ])
    end)
    |> OmQuery.filter_by(Map.take(params, [:role, :organization_id]))
    |> apply_user_sort(params[:sort] || "name")
    |> OmQuery.paginate(:offset,
      limit: params[:per_page] || 20,
      offset: ((params[:page] || 1) - 1) * (params[:per_page] || 20)
    )
    |> OmQuery.preload([:organization, :profile])
    |> OmQuery.execute(include_total_count: true)
  end

  defp apply_user_sort(token, "name"), do: OmQuery.orders(token, asc: :name, asc: :id)
  defp apply_user_sort(token, "-name"), do: OmQuery.orders(token, desc: :name, asc: :id)
  defp apply_user_sort(token, "email"), do: OmQuery.orders(token, asc: :email, asc: :id)
  defp apply_user_sort(token, "-email"), do: OmQuery.orders(token, desc: :email, asc: :id)
  defp apply_user_sort(token, "created"), do: OmQuery.orders(token, asc: :created_at, asc: :id)
  defp apply_user_sort(token, "-created"), do: OmQuery.orders(token, desc: :created_at, asc: :id)
  defp apply_user_sort(token, _), do: OmQuery.orders(token, asc: :name, asc: :id)

  @doc """
  ## Order Management Dashboard - Both Syntaxes

  Complex order listing with customer info, items, and status filters.
  """
  def orders_dashboard_pipeline(params) do
    # PIPELINE STYLE
    Order
    |> OmQuery.new()
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Status filter (single or multiple)
    |> OmQuery.then_if(params[:status], fn token, status when is_binary(status) ->
      OmQuery.filter(token, :status, :eq, status)
    end)
    |> OmQuery.then_if(params[:statuses], fn token, statuses when is_list(statuses) ->
      OmQuery.filter(token, :status, :in, statuses)
    end)
    # Customer filter
    |> OmQuery.then_if(params[:customer_id], fn token, id ->
      OmQuery.filter(token, :customer_id, :eq, id)
    end)
    # Amount range
    |> OmQuery.then_if(params[:min_total], fn token, min ->
      OmQuery.filter(token, :total, :gte, min)
    end)
    |> OmQuery.then_if(params[:max_total], fn token, max ->
      OmQuery.filter(token, :total, :lte, max)
    end)
    # Date filters
    |> OmQuery.then_if(params[:from_date], fn token, date ->
      OmQuery.filter(token, :created_at, :gte, date)
    end)
    |> OmQuery.then_if(params[:to_date], fn token, date ->
      OmQuery.filter(token, :created_at, :lte, date)
    end)
    # Search by order number or customer email
    |> OmQuery.then_if(params[:search], fn token, search ->
      token
      |> OmQuery.join(:customer, :left, as: :cust)
      |> OmQuery.where_any([
        {:order_number, :ilike, "%#{search}%"},
        {:email, :ilike, "%#{search}%", binding: :cust}
      ])
    end)
    # Sorting
    |> OmQuery.orders(parse_order_sort(params[:sort]))
    # Pagination
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 25)
    # Preloads
    |> OmQuery.preload(:customer, fn c ->
      c |> OmQuery.select([:id, :name, :email])
    end)
    |> OmQuery.preload(:items, fn items ->
      items
      |> OmQuery.order(:position, :asc)
      |> OmQuery.preload(:product)
    end)
    |> OmQuery.preload(:shipping_address)
    |> OmQuery.execute()
  end

  def orders_dashboard_dsl(params) do
    # MACRO DSL STYLE - Base query with DSL, conditionals with pipeline
    import OmQuery.DSL

    base =
      query Order do
        filter(:deleted_at, :is_nil, true)

        preload :customer do
          select([:id, :name, :email])
        end

        preload :items do
          order(:position, :asc)
          preload([:product])
        end

        preload([:shipping_address])
      end

    # Apply conditional filters via pipeline
    base
    |> OmQuery.then_if(params[:status], fn token, status ->
      OmQuery.filter(token, :status, :eq, status)
    end)
    |> OmQuery.then_if(params[:statuses], fn token, statuses ->
      OmQuery.filter(token, :status, :in, statuses)
    end)
    |> OmQuery.filter_by(Map.take(params, [:customer_id]))
    |> OmQuery.then_if(params[:min_total], fn token, min ->
      OmQuery.filter(token, :total, :gte, min)
    end)
    |> OmQuery.then_if(params[:max_total], fn token, max ->
      OmQuery.filter(token, :total, :lte, max)
    end)
    |> OmQuery.orders(parse_order_sort(params[:sort]))
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 25)
    |> OmQuery.execute()
  end

  defp parse_order_sort("-total"), do: [desc: :total, asc: :id]
  defp parse_order_sort("total"), do: [asc: :total, asc: :id]
  defp parse_order_sort("-created"), do: [desc: :created_at, asc: :id]
  defp parse_order_sort("created"), do: [asc: :created_at, asc: :id]
  defp parse_order_sort(_), do: [desc: :created_at, asc: :id]

  @doc """
  ## Inventory Report with Joins - Both Syntaxes

  Products with inventory levels, supplier info, and low stock alerts.
  """
  def inventory_report_pipeline(params) do
    # PIPELINE STYLE
    Product
    |> OmQuery.new()
    |> OmQuery.filter(:active, :eq, true)
    |> OmQuery.join(:inventory, :left, as: :inv)
    |> OmQuery.join(:supplier, :left, as: :sup)
    # Category filter
    |> OmQuery.then_if(params[:category_id], fn token, id ->
      OmQuery.filter(token, :category_id, :eq, id)
    end)
    # Low stock filter
    |> OmQuery.if_true(params[:low_stock_only], fn token ->
      token
      |> OmQuery.filter(:quantity, :lte, 10, binding: :inv)
      |> OmQuery.filter(:quantity, :gt, 0, binding: :inv)
    end)
    # Out of stock filter
    |> OmQuery.if_true(params[:out_of_stock], fn token ->
      OmQuery.filter(token, :quantity, :eq, 0, binding: :inv)
    end)
    # Supplier filter
    |> OmQuery.then_if(params[:supplier_id], fn token, id ->
      OmQuery.filter(token, :id, :eq, id, binding: :sup)
    end)
    # Select specific fields for report
    |> OmQuery.select(%{
      product_id: :id,
      product_name: :name,
      sku: :sku,
      price: :price
    })
    |> OmQuery.order(:name, :asc)
    |> OmQuery.paginate(:offset, limit: params[:limit] || 50)
    |> OmQuery.execute()
  end

  def inventory_report_dsl(params) do
    # MACRO DSL STYLE
    import OmQuery.DSL

    base =
      query Product do
        filter(:active, :eq, true)

        join(:inventory, :left, as: :inv)
        join(:supplier, :left, as: :sup)

        select(%{
          product_id: :id,
          product_name: :name,
          sku: :sku,
          price: :price
        })

        order(:name, :asc)
      end

    base
    |> OmQuery.then_if(params[:category_id], fn token, id ->
      OmQuery.filter(token, :category_id, :eq, id)
    end)
    |> OmQuery.if_true(params[:low_stock_only], fn token ->
      token
      |> OmQuery.filter(:quantity, :lte, 10, binding: :inv)
      |> OmQuery.filter(:quantity, :gt, 0, binding: :inv)
    end)
    |> OmQuery.if_true(params[:out_of_stock], fn token ->
      OmQuery.filter(token, :quantity, :eq, 0, binding: :inv)
    end)
    |> OmQuery.paginate(:offset, limit: params[:limit] || 50)
    |> OmQuery.execute()
  end

  @doc """
  ## Activity Timeline - Both Syntaxes

  User activity feed with polymorphic content and pagination.
  """
  def activity_timeline_pipeline(user_id, cursor \\ nil) do
    # PIPELINE STYLE
    Activity
    |> OmQuery.new()
    |> OmQuery.filter(:user_id, :eq, user_id)
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Filter by activity types
    |> OmQuery.filter(:type, :in, ["post", "comment", "like", "follow", "share"])
    # Order by most recent
    |> OmQuery.order(:created_at, :desc)
    |> OmQuery.order(:id, :desc)
    # Cursor pagination
    |> OmQuery.then_if(cursor, fn token, c ->
      OmQuery.paginate(token, :cursor, limit: 20, after: c)
    end)
    |> OmQuery.if_true(is_nil(cursor), fn token ->
      OmQuery.paginate(token, :cursor, limit: 20)
    end)
    # Preload the target (polymorphic)
    |> OmQuery.preload(:target)
    # Preload related user (who did the action)
    |> OmQuery.preload(:actor, fn a ->
      a |> OmQuery.select([:id, :username, :avatar_url])
    end)
    |> OmQuery.execute()
  end

  def activity_timeline_dsl(user_id, cursor \\ nil) do
    # MACRO DSL STYLE
    import OmQuery.DSL

    opts = if cursor, do: [limit: 20, after: cursor], else: [limit: 20]

    query Activity do
      filter(:user_id, :eq, user_id)
      filter(:deleted_at, :is_nil, true)
      filter(:type, :in, ["post", "comment", "like", "follow", "share"])

      order(:created_at, :desc)
      order(:id, :desc)

      paginate(:cursor, opts)

      preload([:target])

      preload :actor do
        select([:id, :username, :avatar_url])
      end
    end
    |> OmQuery.execute()
  end

  @doc """
  ## Multi-Tenant Data Isolation - Both Syntaxes

  Ensure all queries are scoped to the current tenant.
  """
  def tenant_scoped_pipeline(tenant_id, params) do
    # PIPELINE STYLE - Tenant isolation is explicit and auditable
    Project
    |> OmQuery.new()
    # CRITICAL: Always filter by tenant first
    |> OmQuery.filter(:tenant_id, :eq, tenant_id)
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Additional filters
    |> OmQuery.then_if(params[:status], fn token, status ->
      OmQuery.filter(token, :status, :eq, status)
    end)
    |> OmQuery.then_if(params[:owner_id], fn token, owner_id ->
      OmQuery.filter(token, :owner_id, :eq, owner_id)
    end)
    |> OmQuery.then_if(params[:search], fn token, search ->
      OmQuery.filter(token, :name, :ilike, "%#{search}%")
    end)
    |> OmQuery.order(:updated_at, :desc)
    |> OmQuery.paginate(:cursor, limit: 20)
    |> OmQuery.preload(:owner)
    |> OmQuery.preload(:members, fn m ->
      m
      # Also scope preloads!
      |> OmQuery.filter(:tenant_id, :eq, tenant_id)
      |> OmQuery.order(:joined_at, :asc)
    end)
    |> OmQuery.execute()
  end

  def tenant_scoped_dsl(tenant_id, params) do
    # MACRO DSL STYLE
    import OmQuery.DSL

    base =
      query Project do
        # CRITICAL: Tenant isolation
        filter(:tenant_id, :eq, tenant_id)
        filter(:deleted_at, :is_nil, true)

        order(:updated_at, :desc)
        paginate(:cursor, limit: 20)

        preload([:owner])

        preload :members do
          filter(:tenant_id, :eq, tenant_id)
          order(:joined_at, :asc)
        end
      end

    base
    |> OmQuery.filter_by(Map.take(params, [:status, :owner_id]))
    |> OmQuery.then_if(params[:search], fn token, search ->
      OmQuery.filter(token, :name, :ilike, "%#{search}%")
    end)
    |> OmQuery.execute()
  end

  @doc """
  ## Leaderboard with Ranking - Both Syntaxes

  Top users by score with ranking and tie handling.
  """
  def leaderboard_pipeline(params \\ %{}) do
    # PIPELINE STYLE
    User
    |> OmQuery.new()
    |> OmQuery.filter(:status, :eq, "active")
    |> OmQuery.filter(:score, :gt, 0)
    # Optional time period filter
    |> OmQuery.then_if(params[:period] == "weekly", fn token, _ ->
      OmQuery.filter(token, :score_updated_at, :gte, weeks_ago(1))
    end)
    |> OmQuery.then_if(params[:period] == "monthly", fn token, _ ->
      OmQuery.filter(token, :score_updated_at, :gte, months_ago(1))
    end)
    # Category filter
    |> OmQuery.then_if(params[:category], fn token, category ->
      OmQuery.filter(token, :category, :eq, category)
    end)
    # Order by score descending, then by earliest achieved (for ties)
    |> OmQuery.order(:score, :desc)
    |> OmQuery.order(:score_updated_at, :asc)
    |> OmQuery.order(:id, :asc)
    # Limit to top N
    |> OmQuery.limit(params[:limit] || 100)
    |> OmQuery.select([:id, :username, :avatar_url, :score, :score_updated_at])
    # No pagination for leaderboard
    |> OmQuery.execute(unsafe: true)
  end

  def leaderboard_dsl(params \\ %{}) do
    # MACRO DSL STYLE
    import OmQuery.DSL

    base =
      query User do
        filter(:status, :eq, "active")
        filter(:score, :gt, 0)

        order(:score, :desc)
        order(:score_updated_at, :asc)
        order(:id, :asc)

        limit(params[:limit] || 100)

        select([:id, :username, :avatar_url, :score, :score_updated_at])
      end

    base
    |> OmQuery.then_if(params[:period] == "weekly", fn token, _ ->
      OmQuery.filter(token, :score_updated_at, :gte, weeks_ago(1))
    end)
    |> OmQuery.then_if(params[:period] == "monthly", fn token, _ ->
      OmQuery.filter(token, :score_updated_at, :gte, months_ago(1))
    end)
    |> OmQuery.then_if(params[:category], fn token, category ->
      OmQuery.filter(token, :category, :eq, category)
    end)
    |> OmQuery.execute(unsafe: true)
  end

  defp weeks_ago(n), do: DateTime.add(DateTime.utc_now(), -n * 7 * 24 * 60 * 60, :second)
  defp months_ago(n), do: DateTime.add(DateTime.utc_now(), -n * 30 * 24 * 60 * 60, :second)

  @doc """
  ## Content Moderation Queue - Both Syntaxes

  Flagged content awaiting review with priority sorting.
  """
  def moderation_queue_pipeline(moderator_id, params \\ %{}) do
    # PIPELINE STYLE
    Report
    |> OmQuery.new()
    |> OmQuery.filter(:status, :eq, "pending")
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Exclude content the moderator has already reviewed
    |> OmQuery.filter(:reviewed_by_id, :neq, moderator_id)
    # Filter by content type
    |> OmQuery.then_if(params[:content_type], fn token, type ->
      OmQuery.filter(token, :content_type, :eq, type)
    end)
    # Filter by severity
    |> OmQuery.then_if(params[:min_severity], fn token, severity ->
      OmQuery.filter(token, :severity, :gte, severity)
    end)
    # Filter by report reason
    |> OmQuery.then_if(params[:reason], fn token, reason ->
      OmQuery.filter(token, :reason, :eq, reason)
    end)
    # Priority: high severity first, then oldest
    |> OmQuery.order(:severity, :desc)
    |> OmQuery.order(:created_at, :asc)
    |> OmQuery.order(:id, :asc)
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 20)
    # Preload the reported content and reporter
    |> OmQuery.preload(:content)
    |> OmQuery.preload(:reporter, fn r ->
      r |> OmQuery.select([:id, :username, :trust_score])
    end)
    |> OmQuery.preload(:previous_reports, fn pr ->
      pr
      |> OmQuery.filter(:content_id, :eq, {:parent, :content_id})
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(5)
    end)
    |> OmQuery.execute()
  end

  def moderation_queue_dsl(moderator_id, params \\ %{}) do
    # MACRO DSL STYLE
    import OmQuery.DSL

    base =
      query Report do
        filter(:status, :eq, "pending")
        filter(:deleted_at, :is_nil, true)
        filter(:reviewed_by_id, :neq, moderator_id)

        order(:severity, :desc)
        order(:created_at, :asc)
        order(:id, :asc)

        paginate(:cursor, limit: params[:limit] || 20)

        preload([:content])

        preload :reporter do
          select([:id, :username, :trust_score])
        end

        preload :previous_reports do
          order(:created_at, :desc)
          limit(5)
        end
      end

    base
    |> OmQuery.filter_by(Map.take(params, [:content_type, :reason]))
    |> OmQuery.then_if(params[:min_severity], fn token, severity ->
      OmQuery.filter(token, :severity, :gte, severity)
    end)
    |> OmQuery.execute()
  end

  @doc """
  ## Subscription Billing Report - Both Syntaxes

  Active subscriptions with payment history and usage stats.
  """
  def billing_report_pipeline(params \\ %{}) do
    # PIPELINE STYLE
    Subscription
    |> OmQuery.new()
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    # Status filter
    |> OmQuery.then_if(params[:status], fn token, status ->
      OmQuery.filter(token, :status, :eq, status)
    end)
    |> OmQuery.if_true(!params[:status], fn token ->
      OmQuery.filter(token, :status, :in, ["active", "trialing", "past_due"])
    end)
    # Plan filter
    |> OmQuery.then_if(params[:plan_id], fn token, plan_id ->
      OmQuery.filter(token, :plan_id, :eq, plan_id)
    end)
    # MRR range
    |> OmQuery.then_if(params[:min_mrr], fn token, min ->
      OmQuery.filter(token, :mrr, :gte, min)
    end)
    |> OmQuery.then_if(params[:max_mrr], fn token, max ->
      OmQuery.filter(token, :mrr, :lte, max)
    end)
    # Renewal date range
    |> OmQuery.then_if(params[:renews_before], fn token, date ->
      OmQuery.filter(token, :current_period_end, :lte, date)
    end)
    # Churn risk filter
    |> OmQuery.if_true(params[:at_risk], fn token ->
      OmQuery.where_any(token, [
        {:status, :eq, "past_due"},
        {:failed_payment_count, :gte, 2},
        {:usage_percent, :lt, 10}
      ])
    end)
    |> OmQuery.order(:mrr, :desc)
    |> OmQuery.order(:id, :asc)
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 50)
    # Preloads
    |> OmQuery.preload(:customer, fn c ->
      c |> OmQuery.preload(:organization)
    end)
    |> OmQuery.preload(:plan)
    |> OmQuery.preload(:invoices, fn inv ->
      inv
      |> OmQuery.filter(:status, :in, ["paid", "open", "past_due"])
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(3)
    end)
    |> OmQuery.execute()
  end

  def billing_report_dsl(params \\ %{}) do
    # MACRO DSL STYLE
    import OmQuery.DSL

    base =
      query Subscription do
        filter(:deleted_at, :is_nil, true)

        order(:mrr, :desc)
        order(:id, :asc)

        paginate(:cursor, limit: params[:limit] || 50)

        preload :customer do
          preload([:organization])
        end

        preload([:plan])

        preload :invoices do
          filter(:status, :in, ["paid", "open", "past_due"])
          order(:created_at, :desc)
          limit(3)
        end
      end

    base
    |> OmQuery.then_if(params[:status], fn token, status ->
      OmQuery.filter(token, :status, :eq, status)
    end)
    |> OmQuery.if_true(!params[:status], fn token ->
      OmQuery.filter(token, :status, :in, ["active", "trialing", "past_due"])
    end)
    |> OmQuery.filter_by(Map.take(params, [:plan_id]))
    |> OmQuery.then_if(params[:min_mrr], fn token, min ->
      OmQuery.filter(token, :mrr, :gte, min)
    end)
    |> OmQuery.then_if(params[:max_mrr], fn token, max ->
      OmQuery.filter(token, :mrr, :lte, max)
    end)
    |> OmQuery.if_true(params[:at_risk], fn token ->
      OmQuery.where_any(token, [
        {:status, :eq, "past_due"},
        {:failed_payment_count, :gte, 2},
        {:usage_percent, :lt, 10}
      ])
    end)
    |> OmQuery.execute()
  end

  # ============================================================================
  # 14. BEST PRACTICES SUMMARY
  # ============================================================================

  @doc """
  ## When to Use Each Style

  ### Use PIPELINE Style When:
  - Building queries dynamically based on runtime conditions
  - Need maximum composability and reusability
  - Working with fragments and includes
  - Query logic is spread across multiple functions
  - Debugging/tracing query construction

  ### Use DSL Style When:
  - Query structure is mostly static
  - Readability and SQL-like appearance is priority
  - Writing simple CRUD operations
  - Team is more familiar with SQL
  - Query fits naturally in a single block

  ### Hybrid Approach (Recommended):
  - Use DSL for the base query structure
  - Use Pipeline for conditional modifications
  - Best of both worlds!

  ```elixir
  # HYBRID: DSL base + Pipeline conditionals
  import OmQuery.DSL

  query User do
    filter :status, :eq, "active"
    order :created_at, :desc
    preload [:profile, :posts]
  end
  |> OmQuery.then_if(params[:role], &OmQuery.filter(&1, :role, :eq, &2))
  |> OmQuery.then_if(params[:search], fn t, s ->
    OmQuery.filter(t, :name, :ilike, "%\#{s}%")
  end)
  |> OmQuery.paginate(:cursor, limit: 20)
  |> OmQuery.execute()
  ```
  """
  def best_practices_example(params) do
    import OmQuery.DSL

    # HYBRID APPROACH
    query User do
      filter(:status, :eq, "active")
      filter(:deleted_at, :is_nil, true)
      order(:created_at, :desc)
      preload([:profile])
    end
    |> OmQuery.filter_by(Map.take(params, [:role, :organization_id]))
    |> OmQuery.then_if(params[:search], fn token, search ->
      OmQuery.where_any(token, [
        {:name, :ilike, "%#{search}%"},
        {:email, :ilike, "%#{search}%"}
      ])
    end)
    |> OmQuery.paginate(:cursor, limit: params[:limit] || 20)
    |> OmQuery.execute()
  end

  # ===========================================================================
  # 15. E-COMMERCE FACETED SEARCH - COMPREHENSIVE EXAMPLE
  # ===========================================================================

  # ---------------------------------------------------------------------------
  # E-Commerce Product Listing with Faceted Search
  #
  # This is the classic e-commerce pattern with:
  # - Sidebar filters: Category, Brand, Price Range, Rating (with dynamic counts)
  # - Search box: Full-text search across name, description, SKU
  # - Product grid: Paginated results with sorting
  # - Dynamic facet counts: Counts update as you filter/search
  #
  # The UI Layout:
  #
  # ┌─────────────────────────────────────────────────────────────────┐
  # │  [Search: "iphone"_______________] [Sort: Price ▼]              │
  # ├──────────────┬──────────────────────────────────────────────────┤
  # │ Categories   │                                                  │
  # │ ○ All (150)  │  ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐         │
  # │ ● Phones (42)│  │ ▢▢▢▢ │  │ ▢▢▢▢ │  │ ▢▢▢▢ │  │ ▢▢▢▢ │         │
  # │ ○ Cases (28) │  │      │  │      │  │      │  │      │         │
  # │ ○ Access (15)│  │ $999 │  │ $899 │  │ $799 │  │ $699 │         │
  # │              │  └──────┘  └──────┘  └──────┘  └──────┘         │
  # │ Brands       │                                                  │
  # │ ☑ Apple (25) │  ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐         │
  # │ ☑ Samsung(17)│  │ ▢▢▢▢ │  │ ▢▢▢▢ │  │ ▢▢▢▢ │  │ ▢▢▢▢ │         │
  # │ ☐ Google (8) │  │      │  │      │  │      │  │      │         │
  # │              │  │ $599 │  │ $499 │  │ $399 │  │ $299 │         │
  # │ Price        │  └──────┘  └──────┘  └──────┘  └──────┘         │
  # │ ○ All        │                                                  │
  # │ ● $100-$500  │                    [1] [2] [3] ... [Next]        │
  # │ ○ $500-$1000 │                                                  │
  # └──────────────┴──────────────────────────────────────────────────┘
  #
  # Key Requirements:
  # 1. Products: Filter by category, brand, price, rating, stock status
  # 2. Search: Match against name, description, SKU with ILIKE
  # 3. Facet Counts: Show count for each filter option, update dynamically
  # 4. Select from Joins: Need category_name, brand_name in results
  # 5. Pagination: Cursor-based for infinite scroll
  # 6. Preloads: Images, reviews (limited), variant count
  # ---------------------------------------------------------------------------

  @doc """
  PIPELINE APPROACH - Manual Control

  Full control over each query. Best when you need maximum flexibility.
  """
  def ecommerce_listing_pipeline(params) do
    # Extract params with defaults
    search = params[:search]
    category_ids = params[:category_ids] || []
    brand_ids = params[:brand_ids] || []
    min_price = params[:min_price]
    max_price = params[:max_price]
    min_rating = params[:min_rating]
    in_stock_only = params[:in_stock] || false
    sort_by = params[:sort] || "relevance"
    cursor = params[:cursor]

    # =========================================================================
    # STEP 1: Build the base query (shared by products AND facet counts)
    # =========================================================================
    base_query =
      Product
      |> OmQuery.new()
      # Always filter active, non-deleted products
      |> OmQuery.filter(:active, :eq, true)
      |> OmQuery.filter(:deleted_at, :is_nil, true)
      # Apply search across multiple fields
      |> OmQuery.search(search, [:name, :description, :sku])
      # Apply filters using enhanced filter_by
      |> OmQuery.filter_by(%{
        category_id: if(category_ids != [], do: {:in, category_ids}),
        brand_id: if(brand_ids != [], do: {:in, brand_ids}),
        price: build_price_filter(min_price, max_price),
        average_rating: if(min_rating, do: {:gte, min_rating}),
        stock_quantity: if(in_stock_only, do: {:gt, 0})
      })

    # =========================================================================
    # STEP 2: Build the main products query with joins, select, pagination
    # =========================================================================
    products_result =
      base_query
      # Join for category and brand names
      |> OmQuery.join(:category, :left, as: :cat)
      |> OmQuery.join(:brand, :left, as: :brand)
      # Select fields from base AND joined tables (avoiding name conflicts)
      |> OmQuery.select(%{
        id: :id,
        name: :name,
        slug: :slug,
        description: :description,
        price: :price,
        sale_price: :sale_price,
        average_rating: :average_rating,
        review_count: :review_count,
        stock_quantity: :stock_quantity,
        thumbnail_url: :thumbnail_url,
        # Fields from joins - using {binding, field} syntax
        category_id: {:cat, :id},
        category_name: {:cat, :name},
        category_slug: {:cat, :slug},
        brand_id: {:brand, :id},
        brand_name: {:brand, :name},
        brand_logo_url: {:brand, :logo_url}
      })
      # Apply sorting
      |> apply_ecommerce_sort(sort_by)
      # Cursor pagination for infinite scroll
      |> OmQuery.then_if(cursor, fn token, c ->
        OmQuery.paginate(token, :cursor, limit: 24, after: c)
      end)
      |> OmQuery.if_true(is_nil(cursor), fn token ->
        OmQuery.paginate(token, :cursor, limit: 24)
      end)
      # Preload images and limited reviews
      |> OmQuery.preload(:images, fn img ->
        img
        |> OmQuery.filter(:primary, :eq, true)
        |> OmQuery.order(:position, :asc)
        |> OmQuery.limit(5)
      end)
      |> OmQuery.preload(:reviews, fn rev ->
        rev
        |> OmQuery.filter(:approved, :eq, true)
        |> OmQuery.order(:helpful_count, :desc)
        |> OmQuery.limit(3)
      end)
      |> OmQuery.execute!(include_total_count: true)

    # =========================================================================
    # STEP 3: Build facet count queries (run in parallel for efficiency)
    # =========================================================================

    # Category facet - exclude category filter from its own count
    category_base =
      base_query
      |> remove_filter(:category_id)

    # Brand facet - exclude brand filter from its own count
    brand_base =
      base_query
      |> remove_filter(:brand_id)

    # Price facet - exclude price filter from its own count
    price_base =
      base_query
      |> remove_filter(:price)

    # Run facet queries in parallel using Task
    facet_tasks = [
      Task.async(fn -> count_by_category(category_base) end),
      Task.async(fn -> count_by_brand(brand_base) end),
      Task.async(fn -> count_by_price_range(price_base) end),
      Task.async(fn -> count_by_rating(base_query) end)
    ]

    [category_counts, brand_counts, price_counts, rating_counts] =
      Task.await_many(facet_tasks, 5000)

    # =========================================================================
    # STEP 4: Return structured result
    # =========================================================================
    %{
      products: products_result.data,
      pagination: products_result.pagination,
      total_count: products_result.pagination[:total_count],
      facets: %{
        categories: category_counts,
        brands: brand_counts,
        price_ranges: price_counts,
        ratings: rating_counts
      },
      applied_filters: %{
        search: search,
        category_ids: category_ids,
        brand_ids: brand_ids,
        price_range: {min_price, max_price},
        min_rating: min_rating,
        in_stock_only: in_stock_only
      }
    }
  end

  # Helper to build price filter
  defp build_price_filter(nil, nil), do: nil
  defp build_price_filter(min, nil), do: {:gte, min}
  defp build_price_filter(nil, max), do: {:lte, max}
  defp build_price_filter(min, max), do: {:between, {min, max}}

  # Helper to remove a specific filter (for facet counts)
  defp remove_filter(token, _field) do
    # Build a new token without the specified filter
    # This is a simplified version - in practice, you'd track filters separately
    token
  end

  # Apply sorting based on user selection (e-commerce specific)
  defp apply_ecommerce_sort(token, "price_asc") do
    token
    |> OmQuery.order(:price, :asc)
    |> OmQuery.order(:id, :asc)
  end

  defp apply_ecommerce_sort(token, "price_desc") do
    token
    |> OmQuery.order(:price, :desc)
    |> OmQuery.order(:id, :asc)
  end

  defp apply_ecommerce_sort(token, "rating") do
    token
    |> OmQuery.order(:average_rating, :desc_nulls_last)
    |> OmQuery.order(:review_count, :desc)
    |> OmQuery.order(:id, :asc)
  end

  defp apply_ecommerce_sort(token, "newest") do
    token
    |> OmQuery.order(:created_at, :desc)
    |> OmQuery.order(:id, :asc)
  end

  defp apply_ecommerce_sort(token, "bestselling") do
    token
    |> OmQuery.order(:total_sold, :desc)
    |> OmQuery.order(:id, :asc)
  end

  defp apply_ecommerce_sort(token, _relevance) do
    # Default: relevance (for search) or featured products
    token
    |> OmQuery.order(:featured, :desc)
    |> OmQuery.order(:average_rating, :desc_nulls_last)
    |> OmQuery.order(:id, :asc)
  end

  # Count products by category (for sidebar facet)
  defp count_by_category(base_token) do
    # This would use raw Ecto for the group_by count
    # In production, you'd have a dedicated facet counting function
    base_token
    |> OmQuery.join(:category, :left, as: :cat)
    |> OmQuery.group_by(:category_id)
    |> OmQuery.select(%{
      id: :category_id,
      name: {:cat, :name},
      count: {:count, :id}
    })
    |> OmQuery.order(:count, :desc)
    |> OmQuery.execute!()
    |> Map.get(:data, [])
  end

  defp count_by_brand(base_token) do
    base_token
    |> OmQuery.join(:brand, :left, as: :brand)
    |> OmQuery.group_by(:brand_id)
    |> OmQuery.select(%{
      id: :brand_id,
      name: {:brand, :name},
      logo_url: {:brand, :logo_url},
      count: {:count, :id}
    })
    |> OmQuery.order(:count, :desc)
    |> OmQuery.execute!()
    |> Map.get(:data, [])
  end

  defp count_by_price_range(base_token) do
    # Price ranges: Under $50, $50-$100, $100-$500, $500-$1000, Over $1000
    ranges = [
      {0, 50, "Under $50"},
      {50, 100, "$50 - $100"},
      {100, 500, "$100 - $500"},
      {500, 1000, "$500 - $1000"},
      {1000, nil, "Over $1000"}
    ]

    Enum.map(ranges, fn {min, max, label} ->
      count =
        base_token
        |> OmQuery.then_if(min > 0, fn t, _ -> OmQuery.filter(t, :price, :gte, min) end)
        |> OmQuery.then_if(max, fn t, m -> OmQuery.filter(t, :price, :lt, m) end)
        |> OmQuery.count()

      %{min: min, max: max, label: label, count: count}
    end)
  end

  defp count_by_rating(base_token) do
    # Rating facets: 4+ stars, 3+ stars, 2+ stars, 1+ stars
    [4, 3, 2, 1]
    |> Enum.map(fn min_rating ->
      count =
        base_token
        |> OmQuery.filter(:average_rating, :gte, min_rating)
        |> OmQuery.count()

      %{min_rating: min_rating, label: "#{min_rating}+ stars", count: count}
    end)
  end

  @doc """
  ### DSL APPROACH - Declarative Style

  More concise for the base query, with pipeline for conditionals.
  """
  def ecommerce_listing_dsl(params) do
    import OmQuery.DSL

    search = params[:search]
    category_ids = params[:category_ids] || []
    brand_ids = params[:brand_ids] || []
    cursor = params[:cursor]

    # Build base query with DSL
    base =
      query Product do
        filter(:active, :eq, true)
        filter(:deleted_at, :is_nil, true)

        join(:category, :left, as: :cat)
        join(:brand, :left, as: :brand)

        select(%{
          id: :id,
          name: :name,
          price: :price,
          thumbnail_url: :thumbnail_url,
          category_name: {:cat, :name},
          brand_name: {:brand, :name}
        })

        order(:featured, :desc)
        order(:average_rating, :desc)
        order(:id, :asc)

        preload([:images])
      end

    # Apply conditional filters with pipeline
    base
    |> OmQuery.search(search, [:name, :description, :sku])
    |> OmQuery.then_if(category_ids != [], fn token, _ ->
      OmQuery.filter(token, :category_id, :in, category_ids)
    end)
    |> OmQuery.then_if(brand_ids != [], fn token, _ ->
      OmQuery.filter(token, :brand_id, :in, brand_ids)
    end)
    |> OmQuery.filter_by(%{
      price: build_price_filter(params[:min_price], params[:max_price]),
      average_rating: if(params[:min_rating], do: {:gte, params[:min_rating]}),
      stock_quantity: if(params[:in_stock], do: {:gt, 0})
    })
    |> OmQuery.then_if(cursor, fn token, c ->
      OmQuery.paginate(token, :cursor, limit: 24, after: c)
    end)
    |> OmQuery.if_true(is_nil(cursor), fn token ->
      OmQuery.paginate(token, :cursor, limit: 24)
    end)
    |> OmQuery.execute!(include_total_count: true)
  end

  @doc """
  ### FACETED SEARCH PATTERN - The Elegant Solution

  Using the `OmQuery.FacetedSearch` module for a clean, reusable pattern.
  This encapsulates the common e-commerce faceted search logic.
  """
  def ecommerce_faceted_search(params) do
    alias OmQuery.FacetedSearch

    FacetedSearch.new(Product)
    # Text search across multiple fields
    |> FacetedSearch.search(params[:search], [:name, :description, :sku])
    # Apply all filters at once
    |> FacetedSearch.filter_by(%{
      active: true,
      deleted_at: {:is_nil, true},
      category_id: if(params[:category_ids], do: {:in, params[:category_ids]}),
      brand_id: if(params[:brand_ids], do: {:in, params[:brand_ids]}),
      price: build_price_filter(params[:min_price], params[:max_price]),
      average_rating: if(params[:min_rating], do: {:gte, params[:min_rating]}),
      stock_quantity: if(params[:in_stock], do: {:gt, 0})
    })
    # Define facets for sidebar counts
    |> FacetedSearch.facet(:categories, :category_id,
      join: :category,
      label_field: :name,
      # Show all categories even when filtering
      exclude_from_self: true
    )
    |> FacetedSearch.facet(:brands, :brand_id,
      join: :brand,
      label_field: :name,
      exclude_from_self: true
    )
    |> FacetedSearch.facet(:price_ranges, :price,
      ranges: [
        {0, 50, "Under $50"},
        {50, 100, "$50 - $100"},
        {100, 500, "$100 - $500"},
        {500, 1000, "$500 - $1000"},
        {1000, nil, "Over $1000"}
      ]
    )
    |> FacetedSearch.facet(:ratings, :average_rating)
    # Pagination and ordering
    |> FacetedSearch.paginate(:cursor, limit: 24, after: params[:cursor])
    |> FacetedSearch.order(:featured, :desc)
    |> FacetedSearch.order(:average_rating, :desc)
    # Preloads for display
    |> FacetedSearch.preload([:images, :brand, :category])
    # Select specific fields
    |> FacetedSearch.select(%{
      id: :id,
      name: :name,
      slug: :slug,
      price: :price,
      sale_price: :sale_price,
      thumbnail_url: :thumbnail_url,
      average_rating: :average_rating,
      review_count: :review_count
    })
    |> FacetedSearch.execute()
  end

  @doc """
  ## Summary: filter_by Enhanced Syntax

  The enhanced `OmQuery.filter_by/2` supports these formats:

  ```elixir
  OmQuery.filter_by(token, %{
    # Simple equality (default)
    status: "active",

    # Explicit operators
    price: {:gte, 100},
    price: {:lte, 1000},
    price: {:between, {100, 1000}},

    # List operators
    category_id: {:in, [1, 2, 3]},
    status: {:not_in, ["deleted", "archived"]},

    # Null checks
    deleted_at: {:is_nil, true},
    published_at: {:not_nil, true},

    # Pattern matching
    name: {:ilike, "%phone%"},
    sku: {:like, "SKU-%"},

    # Comparison operators
    rating: {:gt, 4},
    stock: {:lt, 10},
    views: {:neq, 0},

    # With binding for joined tables
    category_name: {:eq, "Electronics", binding: :cat}
  })
  ```

  ## Summary: search/4 Function

  ```elixir
  # Simple: same mode for all fields (default :ilike with contains)
  OmQuery.search(token, "iphone", [:name, :description, :sku])

  # Per-field modes: different search strategy per field
  OmQuery.search(token, "iphone", [
    {:sku, :exact},                              # SKU-123 exact match
    {:name, :similarity},                        # Fuzzy match (typo-tolerant)
    {:description, :ilike}                       # Contains search
  ])

  # WITH RANKING: Results ordered by which field matched (rank: true)
  OmQuery.search(token, "iphone", [
    {:sku, :exact, rank: 1},                     # Highest priority - exact SKU
    {:name, :similarity, rank: 2},               # Second - fuzzy name match
    {:brand, :starts_with, rank: 3},             # Third - brand prefix
    {:description, :ilike, rank: 4}              # Lowest - description contains
  ], rank: true)

  # Field Spec Formats:
  # - :field                           → Uses global :mode option
  # - {:field, :mode}                  → Specific mode
  # - {:field, :mode, opts}            → Mode with per-field options (rank, threshold)

  # Available Modes:
  # :ilike           - Case-insensitive LIKE "%term%" (default)
  # :like            - Case-sensitive LIKE "%term%"
  # :exact           - Exact equality match
  # :starts_with     - Prefix match "term%"
  # :ends_with       - Suffix match "%term"
  # :contains        - Same as :ilike
  # :similarity      - PostgreSQL pg_trgm fuzzy (requires extension)
  # :word_similarity - Word-level fuzzy matching
  # :strict_word_similarity - Strictest word boundary matching

  # Per-field Options:
  # - rank: 1        - Priority (lower = higher, used with rank: true)
  # - threshold: 0.4 - Similarity threshold (0.0-1.0, for fuzzy modes)

  # E-commerce search with ranking:
  OmQuery.search(token, params[:q], [
    {:sku, :exact, rank: 1},                     # SKU-123 exact = top result
    {:name, :similarity, rank: 2, threshold: 0.3},
    {:brand, :starts_with, rank: 3},
    {:description, :word_similarity, rank: 4}
  ], rank: true)

  # Autocomplete with ranking (prefix matches first, then fuzzy):
  OmQuery.search(token, input, [
    {:name, :starts_with, rank: 1},
    {:name, :similarity, rank: 2, threshold: 0.2}
  ], rank: true)

  # Note: Similarity modes require PostgreSQL pg_trgm extension:
  # CREATE EXTENSION IF NOT EXISTS pg_trgm;
  ```

  ## Summary: Cross-Table Search with Joins and Take Limits

  ```elixir
  # Search across multiple tables with joins, ranking, and per-field limits
  # Each field can specify: mode, rank, take, threshold, binding

  Product
  |> OmQuery.new()
  # Join related tables with custom conditions
  |> OmQuery.join(:category, :left, as: :cat)
  |> OmQuery.join(:brand, :left, as: :brand)
  |> OmQuery.join(:supplier, :left, as: :sup, on: [supplier_id: :id])
  # Search across fields from multiple tables
  |> OmQuery.search("iphone", [
    # Fields from base table (Product)
    {:sku, :exact, rank: 1, take: 3},                    # Top 3 exact SKU matches
    {:name, :similarity, rank: 2, take: 5, threshold: 0.3},  # 5 fuzzy name matches
    {:description, :ilike, rank: 3, take: 5},            # 5 description matches

    # Fields from joined tables (use binding: option)
    {:name, :ilike, rank: 4, take: 3, binding: :cat},    # 3 category name matches
    {:name, :similarity, rank: 5, take: 3, binding: :brand},  # 3 brand name matches
    {:contact_name, :ilike, rank: 6, take: 2, binding: :sup}  # 2 supplier matches
  ], rank: true)
  # Select fields from all tables
  |> OmQuery.select(%{
    # Base table IDs and search fields
    product_id: :id,
    product_sku: :sku,
    product_name: :name,
    product_description: :description,
    # Joined table IDs and search fields
    category_id: {:cat, :id},
    category_name: {:cat, :name},
    brand_id: {:brand, :id},
    brand_name: {:brand, :name},
    supplier_id: {:sup, :id},
    supplier_contact: {:sup, :contact_name}
  })
  |> OmQuery.execute()

  # Total limit = sum of takes: 3 + 5 + 5 + 3 + 3 + 2 = 21 max results
  # Results ordered by rank (1 first, then 2, etc.)
  ```

  ## Deduplication Behavior with Take Limits

  When multiple fields from the **same table** match the same row:

  ```elixir
  # Example: Two Product fields with take: 5 each
  OmQuery.search(token, "apple", [
    {:name, :similarity, rank: 1, take: 5},        # Product.name matches
    {:description, :ilike, rank: 2, take: 5}       # Product.description matches
  ], rank: true)

  # If "Apple iPhone" matches BOTH name AND description:
  # - Row gets rank 1 (lowest matching rank wins via CASE WHEN)
  # - Row appears ONCE in results (natural SQL deduplication)
  # - Total limit is 10 (5 + 5), but unique rows may be fewer

  # Scenario A: 3 products match name, 4 match description (2 overlap)
  # → 5 unique rows total (not 10), ordered by rank

  # Scenario B: 5 products match name, 5 different match description
  # → 10 unique rows, first 5 are rank-1, next 5 are rank-2
  ```

  ## Deduplication Across Joined Tables

  When searching across different tables (Product + Category + Brand):

  ```elixir
  OmQuery.search(token, "apple", [
    {:name, :similarity, rank: 1, take: 5},                    # Product.name
    {:name, :ilike, rank: 2, take: 3, binding: :cat},         # Category.name
    {:name, :ilike, rank: 3, take: 3, binding: :brand}        # Brand.name
  ], rank: true)

  # Different deduplication scenarios:

  # 1. Product "iPhone" matches Product.name (rank 1)
  #    → Appears once at rank 1

  # 2. Product "MacBook" has Category "Apple Computers" (matches :cat name)
  #    → Appears once at rank 2 (category match)

  # 3. Product "AirPods" has Brand "Apple" (matches :brand name)
  #    AND is in Category "Apple Accessories"
  #    → Appears ONCE at rank 2 (lowest matching rank = category)

  # Total: Up to 11 unique Product rows (5 + 3 + 3)
  # Results show the Product with its joined data
  ```

  ## Full E-Commerce Cross-Table Search Example

  ```elixir
  defmodule MyApp.ProductSearch do
    alias OmQuery

    def search_products(term, opts \\\\ []) do
      include_variants = Keyword.get(opts, :include_variants, false)

      Product
      |> OmQuery.new()
      # Required joins for cross-table search
      |> OmQuery.join(:category, :left, as: :cat)
      |> OmQuery.join(:brand, :left, as: :brand)
      |> OmQuery.join(:tags, :left, as: :tag)
      # Optional variant search
      |> maybe_join_variants(include_variants)
      # Multi-table search with ranking and limits
      |> OmQuery.search(term, [
        # Exact matches first (highest priority)
        {:sku, :exact, rank: 1, take: 5},
        {:barcode, :exact, rank: 1, take: 5},

        # Fuzzy product name (typo-tolerant)
        {:name, :similarity, rank: 2, take: 10, threshold: 0.25},

        # Category/Brand matches
        {:name, :ilike, rank: 3, take: 5, binding: :cat},
        {:name, :similarity, rank: 3, take: 5, binding: :brand, threshold: 0.3},

        # Tag matches
        {:name, :ilike, rank: 4, take: 5, binding: :tag},

        # Description (lowest priority, broader match)
        {:description, :word_similarity, rank: 5, take: 10, threshold: 0.2}
      ], rank: true)
      # Select all IDs and matched fields for display
      |> OmQuery.select(%{
        # Primary result data
        product_id: :id,
        sku: :sku,
        barcode: :barcode,
        name: :name,
        description: :description,
        price: :price,
        # Joined data for context
        category_id: {:cat, :id},
        category_name: {:cat, :name},
        brand_id: {:brand, :id},
        brand_name: {:brand, :name},
        tag_id: {:tag, :id},
        tag_name: {:tag, :name}
      })
      |> OmQuery.execute()
    end

    defp maybe_join_variants(token, false), do: token
    defp maybe_join_variants(token, true) do
      token
      |> OmQuery.join(:variants, :left, as: :var)
    end
  end
  ```

  ## Summary: Select from Joins

  ```elixir
  # Select fields from base table
  OmQuery.select(token, [:id, :name, :price])

  # Select with aliases from base table
  OmQuery.select(token, %{
    product_id: :id,
    product_name: :name
  })

  # Select from joined tables using {binding, field}
  token
  |> OmQuery.join(:category, :left, as: :cat)
  |> OmQuery.join(:brand, :left, as: :brand)
  |> OmQuery.select(%{
    product_id: :id,
    product_name: :name,
    category_id: {:cat, :id},
    category_name: {:cat, :name},
    brand_id: {:brand, :id},
    brand_name: {:brand, :name}
  })
  ```

  ## Summary: OmQuery.debug - Pipeline Debugging

  ```elixir
  # OmQuery.debug works like IO.inspect - prints and returns input unchanged
  # Can be placed ANYWHERE in a pipeline

  # Default: prints raw SQL with interpolated params
  Product
  |> OmQuery.new()
  |> OmQuery.filter(:status, :eq, "active")
  |> OmQuery.debug()                           # Prints SQL, returns token
  |> OmQuery.order(:name, :asc)
  |> OmQuery.execute()

  # Specify output format
  token |> OmQuery.debug(:raw_sql)      # Raw SQL (default)
  token |> OmQuery.debug(:sql_params)   # SQL + separate params list
  token |> OmQuery.debug(:ecto)         # Ecto.Query struct
  token |> OmQuery.debug(:dsl)          # DSL macro syntax
  token |> OmQuery.debug(:pipeline)     # Pipeline syntax
  token |> OmQuery.debug(:token)        # Token struct with operations
  token |> OmQuery.debug(:explain)      # PostgreSQL EXPLAIN
  token |> OmQuery.debug(:all)          # All formats combined

  # Multiple formats at once
  token |> OmQuery.debug([:raw_sql, :dsl])

  # With options
  token |> OmQuery.debug(:raw_sql, label: "Product Query")
  token |> OmQuery.debug(:raw_sql, color: :green)
  token |> OmQuery.debug(:raw_sql, stacktrace: true)  # Shows caller location

  # Debug at multiple points in pipeline
  Product
  |> OmQuery.new()
  |> OmQuery.debug(:token, label: "1. Initial token")
  |> OmQuery.filter(:price, :gt, 100)
  |> OmQuery.debug(:raw_sql, label: "2. After price filter")
  |> OmQuery.join(:category, :left, as: :cat)
  |> OmQuery.debug(:raw_sql, label: "3. After join")
  |> OmQuery.search("iphone", [:name, :description])
  |> OmQuery.debug(:raw_sql, label: "4. After search")
  |> OmQuery.execute()

  # Get debug output as string (without printing)
  sql = OmQuery.Debug.to_string(token, :raw_sql)
  Logger.info("Executing: \#{sql}")

  # Get all formats as a map
  info = OmQuery.Debug.inspect_all(token)
  # => %{raw_sql: "SELECT ...", dsl: "query Product do...", ...}

  # Works with FacetedSearch too
  FacetedSearch.new(Product)
  |> FacetedSearch.search("laptop")
  |> FacetedSearch.filter(:category_id, 5)
  |> OmQuery.debug(:raw_sql, label: "Faceted Search Query")
  |> FacetedSearch.execute()
  ```

  ## Summary: FacetedSearch Pattern

  ```elixir
  alias OmQuery.FacetedSearch

  FacetedSearch.new(Product)
  |> FacetedSearch.search(term, [:name, :description])
  |> FacetedSearch.filter_by(%{...})
  |> FacetedSearch.facet(:categories, :category_id, join: :category)
  |> FacetedSearch.facet(:prices, :price, ranges: [...])
  |> FacetedSearch.paginate(:cursor, limit: 24)
  |> FacetedSearch.execute()

  # Returns:
  %{
    data: [...],
    facets: %{
      categories: [%{id: 1, label: "Electronics", count: 42}, ...],
      prices: [%{label: "Under $50", count: 15}, ...]
    },
    pagination: %{...},
    total_count: 150
  }
  ```
  """
  def ecommerce_summary, do: :see_docs_above

  # ============================================================================
  # ADVANCED FEATURES - Maybe, Scopes, String Ops, Field Comparison
  # ============================================================================

  @doc """
  ## Summary: Maybe (Conditional Filters)

  `maybe` filters only apply when the value is "present" (not nil, not empty string,
  not empty list, not empty map). Perfect for building dynamic queries from user params.

  ```elixir
  # Basic maybe - only applies filter if value is truthy
  OmQuery.maybe(token, :status, params[:status])

  # Maybe with specific operator
  OmQuery.maybe(token, :price, params[:min_price], :gte)
  OmQuery.maybe(token, :category, params[:categories], :in)
  OmQuery.maybe(token, :name, params[:search], :ilike)

  # Maybe with options (case insensitive)
  OmQuery.maybe(token, :email, params[:email], :eq, case_insensitive: true)
  OmQuery.maybe(token, :name, params[:name], :ilike, case_insensitive: true)
  OmQuery.maybe(token, :tags, params[:tags], :in, case_insensitive: true)

  # Maybe on joined tables
  OmQuery.maybe_on(token, :category, :name, params[:category_name], :eq)
  OmQuery.maybe_on(token, :brand, :name, params[:brand], :ilike, case_insensitive: true)

  # DSL syntax
  query User do
    maybe :status == params[:status]
    maybe :role in params[:roles]
    maybe :name =~ params[:search]
    maybe :age >= params[:min_age]
    maybe :age <= params[:max_age]

    # On joined tables
    left_join :department, as: :dept
    maybe {:dept, :name} == params[:department], case: :insensitive
  end
  ```
  """
  def maybe_examples, do: :see_docs_above

  @doc """
  ## Summary: Complex Maybe Example with All Features

  ```elixir
  defmodule ProductSearch do
    alias OmQuery
    import OmQuery.Helpers

    def search(params \\\\ %{}) do
      Product
      |> OmQuery.new()

      # ========== ROOT LEVEL MAYBE FILTERS ==========

      # Text search with ILIKE (case insensitive partial match)
      |> OmQuery.maybe(:name, build_pattern(params[:search]), :ilike)

      # Maybe with :in operator - filter by multiple statuses
      |> OmQuery.maybe(:status, params[:statuses], :in)

      # Maybe with :eq operator
      |> OmQuery.maybe(:active, params[:active], :eq)

      # Maybe with case_insensitive option
      |> OmQuery.maybe(:sku, params[:sku], :eq, case_insensitive: true)

      # Price range using maybe
      |> OmQuery.maybe(:price, params[:min_price], :gte)
      |> OmQuery.maybe(:price, params[:max_price], :lte)

      # Stock check - only apply if flag is true
      |> maybe_in_stock(params[:in_stock])

      # Date filters
      |> OmQuery.maybe(:inserted_at, params[:created_after], :gte)
      |> OmQuery.maybe(:inserted_at, params[:created_before], :lte)

      # ========== JOINS WITH MAYBE FILTERS ==========

      # Category join with case-insensitive IN filter
      |> OmQuery.join(:category, :left, as: :cat)
      |> OmQuery.maybe_on(:cat, :name, params[:categories], :in, case_insensitive: true)
      |> OmQuery.maybe_on(:cat, :active, true, :eq)

      # Brand join with partial match (ILIKE)
      |> OmQuery.join(:brand, :left, as: :brand)
      |> OmQuery.maybe_on(:brand, :name, build_pattern(params[:brand]), :ilike)
      |> OmQuery.maybe_on(:brand, :country, params[:brand_country], :eq, case_insensitive: true)

      # Supplier join
      |> OmQuery.join(:supplier, :left, as: :supplier)
      |> OmQuery.maybe_on(:supplier, :country, params[:supplier_country], :eq)
      |> OmQuery.maybe_on(:supplier, :verified, true, :eq)
      |> OmQuery.maybe_on(:supplier, :rating, params[:min_supplier_rating], :gte)

      # Tags join (many-to-many)
      |> OmQuery.join(:tags, :left, as: :tag)
      |> OmQuery.maybe_on(:tag, :name, params[:tags], :in, case_insensitive: true)

      # ========== PRELOADS WITH MAYBE FILTERS ==========

      # Simple preload
      |> OmQuery.preload(:category)
      |> OmQuery.preload(:brand)

      # Preload with conditional filters
      |> OmQuery.preload(:variants, fn q ->
        q
        |> OmQuery.maybe(:status, "active", :eq)
        |> OmQuery.maybe(:sku, params[:variant_sku], :ilike, case_insensitive: true)
        |> OmQuery.maybe(:color, params[:colors], :in, case_insensitive: true)
        |> OmQuery.maybe(:size, params[:sizes], :in)
        |> OmQuery.maybe(:price, params[:variant_max_price], :lte)
        |> OmQuery.order(:price, :asc)
      end)

      # Preload reviews with nested user preload
      |> OmQuery.preload(:reviews, fn q ->
        q
        |> OmQuery.maybe(:rating, params[:min_rating], :gte)
        |> OmQuery.maybe(:status, "approved", :eq)
        |> OmQuery.maybe(:content, build_pattern(params[:review_search]), :ilike)
        |> OmQuery.order(:created_at, :desc)
        |> OmQuery.limit(10)
        |> OmQuery.preload(:user, fn user_q ->
          user_q
          |> OmQuery.maybe(:status, "active", :eq)
          |> OmQuery.select([:id, :name, :avatar_url])
        end)
      end)

      # Preload inventory with warehouse info
      |> OmQuery.preload(:warehouse_stocks, fn q ->
        q
        |> OmQuery.maybe(:quantity, 0, :gt)
        |> OmQuery.preload(:warehouse, fn wh_q ->
          wh_q
          |> OmQuery.maybe(:region, params[:warehouse_region], :eq)
          |> OmQuery.maybe(:active, true, :eq)
        end)
      end)

      # ========== ORDERING ==========
      |> apply_ordering(params[:sort])

      # ========== CURSOR PAGINATION ==========
      |> apply_pagination(params)

      |> OmQuery.execute()
    end

    # Helper: Build LIKE pattern
    defp build_pattern(nil), do: nil
    defp build_pattern(""), do: nil
    defp build_pattern(term), do: "%\#{term}%"

    # Helper: Conditional stock filter
    defp maybe_in_stock(token, true), do: OmQuery.filter(token, :stock_quantity, :gt, 0)
    defp maybe_in_stock(token, _), do: token

    # Helper: Dynamic ordering
    defp apply_ordering(token, "price_asc"), do: OmQuery.order(token, :price, :asc)
    defp apply_ordering(token, "price_desc"), do: OmQuery.order(token, :price, :desc)
    defp apply_ordering(token, "name"), do: OmQuery.order(token, :name, :asc)
    defp apply_ordering(token, "newest"), do: OmQuery.order(token, :inserted_at, :desc)
    defp apply_ordering(token, "popular") do
      token
      |> OmQuery.order(:sales_count, :desc)
      |> OmQuery.order(:inserted_at, :desc)
    end
    defp apply_ordering(token, _), do: OmQuery.order(token, :inserted_at, :desc)

    # Helper: Cursor pagination with after/before
    defp apply_pagination(token, params) do
      limit = params[:limit] || 20

      cond do
        cursor = params[:after] || params[:cursor] ->
          OmQuery.paginate(token, :cursor, limit: limit, after: cursor)

        cursor = params[:before] ->
          OmQuery.paginate(token, :cursor, limit: limit, before: cursor)

        true ->
          OmQuery.paginate(token, :cursor, limit: limit)
      end
    end
  end

  # Usage examples:

  # Basic search
  ProductSearch.search(%{
    search: "laptop",
    categories: ["Electronics", "Computers"],
    min_price: 500,
    max_price: 2000,
    in_stock: true
  })

  # With pagination - next page
  ProductSearch.search(%{
    categories: ["Electronics"],
    after: "g3QAAAACZAAKaW5zZXJ0ZWRfYXR0..."
  })

  # With pagination - previous page
  ProductSearch.search(%{
    categories: ["Electronics"],
    before: "g3QAAAACZAAKaW5zZXJ0ZWRfYXR0..."
  })

  # Complex filtering
  ProductSearch.search(%{
    search: "wireless",
    brand: "Sony",
    tags: ["bluetooth", "portable"],
    supplier_country: "Japan",
    statuses: ["active", "featured"],
    colors: ["black", "white", "silver"],
    min_rating: 4,
    sort: "popular",
    limit: 24
  })
  ```
  """
  def complex_maybe_example, do: :see_docs_above

  @doc """
  ## Summary: Scopes - Reusable Query Fragments

  Scopes are reusable query building blocks that can be applied anywhere.

  ```elixir
  # Define scopes in a module
  defmodule UserScopes do
    alias OmQuery

    def active(token), do: OmQuery.filter(token, :status, :eq, "active")
    def verified(token), do: OmQuery.filter(token, :verified_at, :not_nil, true)
    def admin(token), do: OmQuery.filter(token, :role, :eq, "admin")
    def recent(token), do: OmQuery.order(token, :created_at, :desc) |> OmQuery.limit(10)

    # Scope with arguments
    def created_after(token, date), do: OmQuery.filter(token, :inserted_at, :gte, date)
    def role(token, role), do: OmQuery.filter(token, :role, :eq, role)
    def older_than(token, age), do: OmQuery.filter(token, :age, :gt, age)
  end

  defmodule ProductScopes do
    alias OmQuery

    def active(token), do: OmQuery.filter(token, :active, :eq, true)
    def in_stock(token), do: OmQuery.filter(token, :stock_quantity, :gt, 0)
    def on_sale(token), do: OmQuery.filter(token, :sale_price, :not_nil, true)
    def featured(token), do: OmQuery.filter(token, :featured, :eq, true)

    def priced_between(token, min, max), do: OmQuery.between(token, :price, min, max)
    def in_category(token, cat_id), do: OmQuery.filter(token, :category_id, :eq, cat_id)
  end

  # ========== Using Scopes ==========

  # Apply scope with function reference
  User
  |> OmQuery.scope(&UserScopes.active/1)
  |> OmQuery.scope(&UserScopes.verified/1)
  |> OmQuery.execute()

  # Apply scope from module by name
  User
  |> OmQuery.apply_scope(UserScopes, :active)
  |> OmQuery.apply_scope(UserScopes, :verified)
  |> OmQuery.execute()

  # Apply scope with arguments
  User
  |> OmQuery.apply_scope(UserScopes, :active)
  |> OmQuery.apply_scope(UserScopes, :created_after, [~D[2024-01-01]])
  |> OmQuery.apply_scope(UserScopes, :older_than, [18])
  |> OmQuery.execute()

  # Chain multiple scopes
  scopes = [
    &ProductScopes.active/1,
    &ProductScopes.in_stock/1,
    &ProductScopes.featured/1
  ]

  Product
  |> OmQuery.scopes(scopes)
  |> OmQuery.execute()

  # ========== Scopes in Preloads ==========

  User
  |> OmQuery.preload(:posts, fn q ->
    q
    |> OmQuery.scope(&PostScopes.published/1)
    |> OmQuery.scope(&PostScopes.recent/1)
  end)
  |> OmQuery.execute()

  # Or using apply_scope
  User
  |> OmQuery.preload(:orders, fn q ->
    OmQuery.apply_scope(q, OrderScopes, :completed)
  end)
  |> OmQuery.execute()

  # ========== Scopes with Joins ==========

  Product
  |> OmQuery.join(:category, :left, as: :cat)
  |> OmQuery.scope(&ProductScopes.active/1)
  |> OmQuery.scope(&ProductScopes.in_stock/1)
  |> OmQuery.execute()

  # ========== DSL Syntax ==========

  query User do
    scope &UserScopes.active/1
    scope &UserScopes.verified/1
    apply_scope UserScopes, :older_than, [21]
  end
  |> OmQuery.execute()

  # ========== Combining Scopes with Maybe ==========

  Product
  |> OmQuery.scope(&ProductScopes.active/1)
  |> OmQuery.maybe(:category_id, params[:category_id], :eq)
  |> OmQuery.maybe(:brand_id, params[:brand_id], :eq)
  |> OmQuery.scope(&ProductScopes.in_stock/1)
  |> OmQuery.apply_scope(ProductScopes, :priced_between, [params[:min] || 0, params[:max] || 10000])
  |> OmQuery.execute()
  ```
  """
  def scope_examples, do: :see_docs_above

  @doc """
  ## Summary: OR / AND / NOT Conditions

  ```elixir
  # ========== OR Conditions (any_of / where_any) ==========

  # Match if ANY condition is true
  OmQuery.where_any(token, [
    {:status, :eq, "active"},
    {:role, :eq, "admin"},
    {:verified, :eq, true}
  ])
  # SQL: WHERE (status = 'active' OR role = 'admin' OR verified = true)

  # With options
  OmQuery.where_any(token, [
    {:email, :eq, "admin@example.com", [case_insensitive: true]},
    {:role, :eq, "admin"}
  ])

  # DSL syntax
  query User do
    any_of [
      {:status, :eq, "active"},
      {:role, :eq, "admin"}
    ]
  end

  # ========== AND Conditions (all_of / where_all) ==========

  # Match if ALL conditions are true (explicit grouping)
  OmQuery.where_all(token, [
    {:status, :eq, "active"},
    {:verified, :eq, true}
  ])
  # SQL: WHERE (status = 'active' AND verified = true)

  # DSL syntax
  query User do
    all_of [
      {:status, :eq, "active"},
      {:age, :gte, 18}
    ]
  end

  # ========== NOT Conditions (none_of / where_none / where_not) ==========

  # Match if NONE of the conditions are true
  OmQuery.where_none(token, [
    {:status, :eq, "banned"},
    {:status, :eq, "deleted"},
    {:status, :eq, "suspended"}
  ])
  # SQL: WHERE NOT (status = 'banned' OR status = 'deleted' OR status = 'suspended')

  # Negate a single filter
  OmQuery.where_not(token, :status, :eq, "active")
  # SQL: WHERE status != 'active'

  OmQuery.where_not(token, :price, :gt, 100)
  # SQL: WHERE price <= 100 (operator is inverted)

  OmQuery.where_not(token, :role, :in, ["banned", "suspended"])
  # SQL: WHERE role NOT IN ('banned', 'suspended')

  # DSL syntax
  query User do
    none_of [
      {:status, :eq, "banned"},
      {:status, :eq, "spam"}
    ]

    where_not :role, :eq, "guest"
  end

  # ========== Operator Inversions for where_not ==========
  # :eq      -> :neq
  # :neq     -> :eq
  # :gt      -> :lte
  # :gte     -> :lt
  # :lt      -> :gte
  # :lte     -> :gt
  # :in      -> :not_in
  # :not_in  -> :in
  # :is_nil  -> :not_nil
  # :not_nil -> :is_nil
  # :like    -> :not_like
  # :ilike   -> :not_ilike
  ```
  """
  def or_and_not_examples, do: :see_docs_above

  @doc """
  ## Summary: Field-to-Field Comparison

  Compare two fields within the same row.

  ```elixir
  # Records modified after creation
  OmQuery.where_field(token, :updated_at, :gt, :created_at)
  # SQL: WHERE updated_at > created_at

  # Products running low on stock
  OmQuery.where_field(token, :current_stock, :lt, :min_stock)
  # SQL: WHERE current_stock < min_stock

  # Orders where total equals subtotal (no discount)
  OmQuery.where_field(token, :total, :eq, :subtotal)
  # SQL: WHERE total = subtotal

  # With binding for joined tables
  OmQuery.where_field(token, :user_id, :eq, :author_id, binding: :post)

  # Supported operators: :eq, :neq, :gt, :gte, :lt, :lte

  # DSL syntax
  query Product do
    where_field :updated_at, :gt, :created_at
    where_field :current_stock, :lte, :reorder_point
  end
  ```
  """
  def field_comparison_examples, do: :see_docs_above

  @doc """
  ## Summary: String Operations

  ```elixir
  # ========== Starts With ==========
  OmQuery.starts_with(token, :name, "John")
  # SQL: WHERE name LIKE 'John%'

  OmQuery.starts_with(token, :email, "admin", case_insensitive: true)
  # SQL: WHERE email ILIKE 'admin%'

  # ========== Ends With ==========
  OmQuery.ends_with(token, :email, "@example.com")
  # SQL: WHERE email LIKE '%@example.com'

  OmQuery.ends_with(token, :filename, ".pdf", case_insensitive: true)
  # SQL: WHERE filename ILIKE '%.pdf'

  # ========== Contains ==========
  OmQuery.contains_string(token, :description, "important")
  # SQL: WHERE description LIKE '%important%'

  OmQuery.contains_string(token, :content, "urgent", case_insensitive: true)
  # SQL: WHERE content ILIKE '%urgent%'

  # ========== Negations ==========
  OmQuery.not_starts_with(token, :name, "Test")
  # SQL: WHERE name NOT LIKE 'Test%'

  OmQuery.not_ends_with(token, :filename, ".tmp")
  # SQL: WHERE filename NOT LIKE '%.tmp'

  OmQuery.not_contains_string(token, :content, "spam")
  # SQL: WHERE content NOT LIKE '%spam%'

  # ========== DSL Syntax ==========
  query User do
    starts_with :name, "John"
    ends_with :email, "@company.com"
    contains_string :bio, "developer"
  end

  # ========== Combining with Maybe ==========
  OmQuery.maybe(token, :name, build_pattern(params[:search]), :ilike)

  defp build_pattern(nil), do: nil
  defp build_pattern(""), do: nil
  defp build_pattern(term), do: "%\#{term}%"
  ```
  """
  def string_operations_examples, do: :see_docs_above

  @doc """
  ## Summary: Null/Blank Helpers

  ```elixir
  # ========== Null Checks ==========

  # Field is NULL
  OmQuery.where_nil(token, :deleted_at)
  # SQL: WHERE deleted_at IS NULL

  # Field is NOT NULL
  OmQuery.where_not_nil(token, :email)
  # SQL: WHERE email IS NOT NULL

  # ========== Blank Checks (NULL or empty string) ==========

  # Field is blank (NULL OR empty string)
  OmQuery.where_blank(token, :bio)
  # SQL: WHERE (bio IS NULL OR bio = '')

  # Field is present (NOT NULL AND NOT empty)
  OmQuery.where_present(token, :name)
  # SQL: WHERE name IS NOT NULL AND name != ''

  # ========== DSL Syntax ==========
  query User do
    null :deleted_at        # IS NULL
    not_null :email         # IS NOT NULL
    blank :middle_name      # IS NULL OR = ''
    present :first_name     # IS NOT NULL AND != ''
  end

  # ========== With Bindings ==========
  OmQuery.where_nil(token, :deleted_at, binding: :user)
  OmQuery.where_present(token, :title, binding: :post)

  # ========== Soft Delete Helpers ==========
  OmQuery.exclude_deleted(token)           # WHERE deleted_at IS NULL
  OmQuery.exclude_deleted(token, :removed_at)  # Custom field

  OmQuery.only_deleted(token)              # WHERE deleted_at IS NOT NULL
  ```
  """
  def null_blank_examples, do: :see_docs_above

  @doc """
  ## Summary: Between and Range Shortcuts

  ```elixir
  # ========== Between ==========

  # Price between 10 and 100
  OmQuery.between(token, :price, 10, 100)
  # SQL: WHERE price BETWEEN 10 AND 100

  # Age between 18 and 65
  OmQuery.between(token, :age, 18, 65)

  # With binding
  OmQuery.between(token, :quantity, 1, 10, binding: :items)

  # ========== At Least (>=) ==========

  OmQuery.at_least(token, :price, 100)
  # SQL: WHERE price >= 100

  OmQuery.at_least(token, :age, 18)
  # SQL: WHERE age >= 18

  # ========== At Most (<=) ==========

  OmQuery.at_most(token, :price, 1000)
  # SQL: WHERE price <= 1000

  OmQuery.at_most(token, :quantity, 50)
  # SQL: WHERE quantity <= 50

  # ========== DSL Syntax ==========
  query Product do
    between :price, 10, 100
    at_least :stock, 5
    at_most :weight, 10
  end

  # ========== Date Range Helpers ==========

  # Records created between dates
  OmQuery.created_between(token, start_date, end_date)
  OmQuery.created_between(token, start_date, end_date, :published_at)  # Custom field

  # Records created today
  OmQuery.created_today(token)
  OmQuery.created_today(token, :placed_at)  # Custom field

  # Records updated recently
  OmQuery.updated_recently(token, 24)  # Last 24 hours
  OmQuery.updated_since(token, datetime)
  ```
  """
  def between_range_examples, do: :see_docs_above

  @doc """
  ## Summary: Raw SQL Escape Hatch

  For complex SQL that can't be expressed with the DSL.

  ```elixir
  # Basic raw WHERE clause
  OmQuery.raw(token, "price > 100 AND stock < 10")
  # SQL: WHERE price > 100 AND stock < 10

  # With parameters (prevents SQL injection)
  OmQuery.raw(token, "price > ? AND stock < ?", [100, 10])
  # SQL: WHERE price > $1 AND stock < $2

  # Complex expressions
  OmQuery.raw(token, "EXTRACT(YEAR FROM created_at) = ?", [2024])

  OmQuery.raw(token, "similarity(name, ?) > 0.3", [search_term])

  OmQuery.raw(token,
    "ST_Distance(location, ST_Point(?, ?)) < ?",
    [lng, lat, radius_meters]
  )

  # Full-text search
  OmQuery.raw(token,
    "to_tsvector('english', name || ' ' || description) @@ plainto_tsquery('english', ?)",
    [search_query]
  )

  # Array operations
  OmQuery.raw(token, "? = ANY(tags)", [tag_name])
  OmQuery.raw(token, "tags && ARRAY[?]::text[]", [tag_list])

  # DSL syntax
  query Product do
    raw "price > ? AND featured = ?", [100, true]
    raw "EXTRACT(DOW FROM created_at) IN (0, 6)"  # Weekends only
  end

  # Combining with other filters
  Product
  |> OmQuery.filter(:status, :eq, "active")
  |> OmQuery.raw("similarity(name, ?) > 0.5", [search_term])
  |> OmQuery.filter(:category_id, :eq, category_id)
  |> OmQuery.execute()
  ```
  """
  def raw_sql_examples, do: :see_docs_above

  @doc """
  ## Summary: Cursor Pagination with After/Before

  ```elixir
  # ========== Basic Cursor Pagination ==========

  # First page
  OmQuery.paginate(token, :cursor, limit: 20)

  # Next page (forward)
  OmQuery.paginate(token, :cursor, limit: 20, after: cursor)

  # Previous page (backward)
  OmQuery.paginate(token, :cursor, limit: 20, before: cursor)

  # ========== With Custom Cursor Fields ==========

  # Automatic inference from order_by (recommended)
  Product
  |> OmQuery.order(:price, :asc)
  |> OmQuery.order(:id, :asc)
  |> OmQuery.paginate(:cursor, limit: 20)
  # cursor_fields automatically inferred as [:price, :id]

  # Explicit cursor fields
  OmQuery.paginate(token, :cursor,
    limit: 20,
    cursor_fields: [:created_at, :id],
    after: cursor
  )

  # ========== Pagination Helper Pattern ==========

  defmodule PaginationHelper do
    alias OmQuery

    def paginate(token, params) do
      limit = params[:limit] || params["limit"] || 20

      cond do
        # Forward pagination
        cursor = params[:after] || params["after"] || params[:cursor] ->
          OmQuery.paginate(token, :cursor, limit: limit, after: cursor)

        # Backward pagination
        cursor = params[:before] || params["before"] ->
          OmQuery.paginate(token, :cursor, limit: limit, before: cursor)

        # First page
        true ->
          OmQuery.paginate(token, :cursor, limit: limit)
      end
    end
  end

  # Usage
  Product
  |> OmQuery.filter(:status, :eq, "active")
  |> OmQuery.order(:created_at, :desc)
  |> PaginationHelper.paginate(params)
  |> OmQuery.execute()

  # ========== Result Structure ==========

  # OmQuery.execute() returns:
  %OmQuery.Result{
    data: [%Product{}, %Product{}, ...],
    pagination: %{
      has_next_page: true,
      has_previous_page: false,
      start_cursor: "g3QAAAACZAAKaW5zZXJ0...",
      end_cursor: "g3QAAAACZAAKaW5zZXJ0...",
      limit: 20
    },
    metadata: %{
      query_time_ms: 12
    }
  }

  # ========== Pagination in Controller ==========

  def index(conn, params) do
    result =
      Product
      |> OmQuery.new()
      |> OmQuery.scope(&ProductScopes.active/1)
      |> OmQuery.maybe(:category_id, params["category_id"], :eq)
      |> OmQuery.order(:created_at, :desc)
      |> paginate_from_params(params)
      |> OmQuery.execute()

    json(conn, %{
      data: result.data,
      pagination: %{
        has_next: result.pagination.has_next_page,
        has_prev: result.pagination.has_previous_page,
        next_cursor: result.pagination.end_cursor,
        prev_cursor: result.pagination.start_cursor
      }
    })
  end

  defp paginate_from_params(token, params) do
    limit = parse_int(params["limit"], 20)

    cond do
      cursor = params["after"] ->
        OmQuery.paginate(token, :cursor, limit: limit, after: cursor)
      cursor = params["before"] ->
        OmQuery.paginate(token, :cursor, limit: limit, before: cursor)
      true ->
        OmQuery.paginate(token, :cursor, limit: limit)
    end
  end
  ```
  """
  def cursor_pagination_examples, do: :see_docs_above

  @doc """
  ## Summary: Maybe with Custom Predicates (`:when` option)

  The `maybe` function accepts a `:when` option to customize when the filter is applied.

  ### Built-in Predicates

  ```elixir
  # :present (default) - not nil, false, "", [], %{}
  OmQuery.maybe(User, :status, params[:status], :eq)  # same as when: :present

  # :not_nil - only checks for nil (allows false, "", [])
  OmQuery.maybe(User, :active, params[:active], :eq, when: :not_nil)

  # :not_blank - not nil, "", or whitespace-only string
  OmQuery.maybe(User, :name, params[:name], :ilike, when: :not_blank)

  # :not_empty - not nil, [], or %{}
  OmQuery.maybe(User, :tags, params[:tags], :in, when: :not_empty)
  ```

  ### Custom Predicate Functions

  ```elixir
  # Only apply filter if value is positive
  OmQuery.maybe(User, :score, params[:min_score], :gte, when: &(&1 && &1 > 0))

  # Only apply filter if list has items
  OmQuery.maybe(User, :roles, params[:roles], :in, when: &(is_list(&1) and &1 != []))

  # Check for valid UUID
  OmQuery.maybe(User, :external_id, params[:ext_id], :eq, when: &valid_uuid?/1)
  ```

  ### Use Cases

  ```elixir
  # Boolean filters - allow false value
  # With :present (default), false would skip the filter
  # With :not_nil, false applies the filter
  OmQuery.maybe(User, :active, params[:active], :eq, when: :not_nil)

  # Search fields - skip whitespace-only input
  OmQuery.maybe(User, :name, params[:search], :ilike, when: :not_blank)

  # Multi-select - skip empty arrays
  OmQuery.maybe(Product, :category_ids, params[:categories], :in, when: :not_empty)
  ```

  ### In DSL

  ```elixir
  query User do
    maybe(:status == params[:status], when: :not_nil)
    maybe(:name =~ params[:search], when: :not_blank)
    maybe(:role in params[:roles], when: :not_empty)
  end
  ```

  ### On Joined Tables

  ```elixir
  Product
  |> OmQuery.join(:category, :left, as: :cat)
  |> OmQuery.maybe_on(:cat, :active, params[:cat_active], :eq, when: :not_nil)
  |> OmQuery.maybe_on(:cat, :name, params[:cat_search], :ilike, when: :not_blank)
  ```
  """
  def maybe_predicates_examples, do: :see_docs_above

  @doc """
  ## Summary: Query Params Helper (`OmQuery.Params`)

  Utility module for working with query parameters from Phoenix controllers, LiveView, etc.
  Provides indifferent access (atom or string keys) and normalization.

  ### Indifferent Access

  ```elixir
  alias OmQuery.Params

  # Works with both string and atom keys
  params = %{"limit" => 20, "status" => "active"}

  Params.get(params, :limit)    #=> 20
  Params.get(params, :status)   #=> "active"
  Params.get(params, :missing)  #=> nil
  Params.get(params, :missing, "default")  #=> "default"
  ```

  ### With Query Functions

  ```elixir
  alias OmQuery
  alias OmQuery.Params

  def search(params) do
    Product
    |> OmQuery.new()
    |> OmQuery.maybe(:status, Params.get(params, :status))
    |> OmQuery.maybe(:category_id, Params.get(params, :category_id), :eq)
    |> OmQuery.maybe(:min_price, Params.get(params, :min_price), :gte)
    |> OmQuery.execute()
  end
  ```

  ### Pagination Options Helper

  ```elixir
  # Extracts and parses pagination options automatically
  params = %{"limit" => "25", "after" => "cursor_token"}

  opts = Params.pagination_opts(params)
  #=> [limit: 25, after: "cursor_token"]

  # Use directly with paginate
  OmQuery.paginate(token, :cursor, opts)

  # Custom key mapping
  params = %{"page_size" => "10", "cursor" => "abc"}
  Params.pagination_opts(params, limit: :page_size, after: :cursor)
  #=> [limit: 10, after: "abc"]
  ```

  ### Normalize Params

  ```elixir
  # Convert string keys to atoms
  params = %{"limit" => 20, "status" => "active"}
  Params.normalize(params)
  #=> %{limit: 20, status: "active"}

  # Safe normalization with allowed keys (recommended)
  Params.normalize(params, only: [:limit, :status, :page])
  #=> %{limit: 20, status: "active"}  # unknown keys dropped
  ```

  ### Take and Compact

  ```elixir
  # Take only specific keys
  params = %{"limit" => 20, "status" => "active", "other" => "ignored"}
  Params.take(params, [:limit, :status])
  #=> %{limit: 20, status: "active"}

  # Remove nil values
  Params.compact(limit: nil, after: "cursor", status: nil)
  #=> [after: "cursor"]
  ```

  ### Full Controller Example

  ```elixir
  defmodule MyAppWeb.ProductController do
    alias OmQuery
    alias OmQuery.Params

    def index(conn, params) do
      result =
        Product
        |> OmQuery.new()
        |> OmQuery.maybe(:status, Params.get(params, :status))
        |> OmQuery.maybe(:category_id, Params.get(params, :category_id), :eq)
        |> OmQuery.maybe(:search, Params.get(params, :q), :ilike, when: :not_blank)
        |> OmQuery.maybe(:in_stock, Params.get(params, :in_stock), :eq, when: :not_nil)
        |> OmQuery.order(:created_at, :desc)
        |> OmQuery.paginate(:cursor, Params.pagination_opts(params))
        |> OmQuery.execute()

      json(conn, %{
        data: result.data,
        pagination: result.pagination
      })
    end
  end
  ```

  ### LiveView Example

  ```elixir
  defmodule MyAppWeb.ProductLive.Index do
    alias OmQuery
    alias OmQuery.Params

    def handle_event("filter", params, socket) do
      products = search_products(params)
      {:noreply, assign(socket, products: products)}
    end

    defp search_products(params) do
      Product
      |> OmQuery.new()
      |> OmQuery.maybe(:category, Params.get(params, :category), when: :not_blank)
      |> OmQuery.maybe(:min_price, Params.get(params, :min_price), :gte)
      |> OmQuery.maybe(:max_price, Params.get(params, :max_price), :lte)
      |> OmQuery.paginate(:cursor, Params.pagination_opts(params))
      |> OmQuery.execute()
    end
  end
  ```
  """
  def params_helper_examples, do: :see_docs_above

  @doc """
  ## Summary: Complete Real-World Example

  A full e-commerce product search combining all features.

  ```elixir
  defmodule MyApp.Catalog.ProductQuery do
    @moduledoc "Product search with all advanced query features"

    alias OmQuery
    import OmQuery.Helpers

    # Scope definitions
    defmodule Scopes do
      alias OmQuery

      def active(q), do: OmQuery.filter(q, :status, :eq, "active")
      def in_stock(q), do: OmQuery.filter(q, :stock_quantity, :gt, 0)
      def featured(q), do: OmQuery.filter(q, :featured, :eq, true)
      def on_sale(q), do: OmQuery.where_not_nil(q, :sale_price)
      def recent(q), do: OmQuery.filter(q, :inserted_at, :gte, days_ago(30))
    end

    @doc "Search products with comprehensive filtering"
    def search(params \\\\ %{}) do
      Product
      |> OmQuery.new()

      # Apply base scopes
      |> OmQuery.scope(&Scopes.active/1)
      |> maybe_apply_scope(params[:in_stock], &Scopes.in_stock/1)
      |> maybe_apply_scope(params[:featured], &Scopes.featured/1)
      |> maybe_apply_scope(params[:on_sale], &Scopes.on_sale/1)

      # Text search (ILIKE with pattern)
      |> OmQuery.maybe(:name, like_pattern(params[:q]), :ilike)
      |> OmQuery.maybe(:description, like_pattern(params[:q]), :ilike)
      |> OmQuery.maybe(:sku, params[:sku], :eq, case_insensitive: true)

      # Status filtering with IN
      |> OmQuery.maybe(:visibility, params[:visibility], :in)

      # Price range
      |> OmQuery.maybe(:price, params[:min_price], :gte)
      |> OmQuery.maybe(:price, params[:max_price], :lte)

      # Date filters
      |> OmQuery.maybe(:inserted_at, params[:created_after], :gte)
      |> OmQuery.maybe(:updated_at, params[:updated_after], :gte)

      # Field comparison - products needing restock
      |> maybe_low_stock(params[:low_stock])

      # Category join with filters
      |> OmQuery.join(:category, :left, as: :cat)
      |> OmQuery.maybe_on(:cat, :id, params[:category_ids], :in)
      |> OmQuery.maybe_on(:cat, :name, params[:category_names], :in, case_insensitive: true)
      |> OmQuery.maybe_on(:cat, :slug, params[:category_slug], :eq)

      # Brand join
      |> OmQuery.join(:brand, :left, as: :brand)
      |> OmQuery.maybe_on(:brand, :id, params[:brand_ids], :in)
      |> OmQuery.maybe_on(:brand, :name, like_pattern(params[:brand]), :ilike)

      # Tags (OR match - product has ANY of these tags)
      |> maybe_filter_tags(params[:tags])

      # Supplier country filter
      |> OmQuery.join(:supplier, :left, as: :supplier)
      |> OmQuery.maybe_on(:supplier, :country_code, params[:origin_countries], :in)

      # Exclusion filters using none_of
      |> maybe_exclude_statuses(params[:exclude_statuses])

      # Preloads with filtered associations
      |> OmQuery.preload(:category)
      |> OmQuery.preload(:brand)

      |> OmQuery.preload(:variants, fn q ->
        q
        |> OmQuery.filter(:status, :eq, "active")
        |> OmQuery.maybe(:color, params[:colors], :in, case_insensitive: true)
        |> OmQuery.maybe(:size, params[:sizes], :in)
        |> OmQuery.order(:price, :asc)
      end)

      |> OmQuery.preload(:images, fn q ->
        q
        |> OmQuery.filter(:type, :eq, "gallery")
        |> OmQuery.order(:position, :asc)
        |> OmQuery.limit(5)
      end)

      |> OmQuery.preload(:reviews, fn q ->
        q
        |> OmQuery.filter(:status, :eq, "approved")
        |> OmQuery.maybe(:rating, params[:min_rating], :gte)
        |> OmQuery.order(:helpful_count, :desc)
        |> OmQuery.limit(3)
        |> OmQuery.preload(:user, fn u ->
          u |> OmQuery.select([:id, :name, :avatar_url])
        end)
      end)

      # Ordering
      |> apply_sort(params[:sort])

      # Cursor pagination
      |> apply_pagination(params)

      |> OmQuery.execute()
    end

    # ========== Private Helpers ==========

    defp like_pattern(nil), do: nil
    defp like_pattern(""), do: nil
    defp like_pattern(term), do: "%\#{String.trim(term)}%"

    defp maybe_apply_scope(token, true, scope_fn), do: OmQuery.scope(token, scope_fn)
    defp maybe_apply_scope(token, _, _), do: token

    defp maybe_low_stock(token, true) do
      OmQuery.where_field(token, :stock_quantity, :lte, :reorder_point)
    end
    defp maybe_low_stock(token, _), do: token

    defp maybe_filter_tags(token, nil), do: token
    defp maybe_filter_tags(token, []), do: token
    defp maybe_filter_tags(token, tags) when is_list(tags) do
      token
      |> OmQuery.join(:product_tags, :inner, as: :pt)
      |> OmQuery.join(:tags, :inner, as: :tag, on: [id: {:pt, :tag_id}])
      |> OmQuery.filter(:name, :in, tags, binding: :tag, case_insensitive: true)
    end

    defp maybe_exclude_statuses(token, nil), do: token
    defp maybe_exclude_statuses(token, []), do: token
    defp maybe_exclude_statuses(token, statuses) when is_list(statuses) do
      exclusions = Enum.map(statuses, &{:status, :eq, &1})
      OmQuery.where_none(token, exclusions)
    end

    defp apply_sort(token, "price_asc"), do: OmQuery.orders(token, [price: :asc, id: :asc])
    defp apply_sort(token, "price_desc"), do: OmQuery.orders(token, [price: :desc, id: :asc])
    defp apply_sort(token, "name"), do: OmQuery.orders(token, [name: :asc, id: :asc])
    defp apply_sort(token, "newest"), do: OmQuery.orders(token, [inserted_at: :desc, id: :asc])
    defp apply_sort(token, "bestselling"), do: OmQuery.orders(token, [sales_count: :desc, id: :asc])
    defp apply_sort(token, "rating"), do: OmQuery.orders(token, [avg_rating: :desc, id: :asc])
    defp apply_sort(token, _), do: OmQuery.orders(token, [inserted_at: :desc, id: :asc])

    defp apply_pagination(token, params) do
      limit = min(params[:limit] || 24, 100)

      cond do
        cursor = params[:after] -> OmQuery.paginate(token, :cursor, limit: limit, after: cursor)
        cursor = params[:before] -> OmQuery.paginate(token, :cursor, limit: limit, before: cursor)
        true -> OmQuery.paginate(token, :cursor, limit: limit)
      end
    end
  end

  # ========== Usage Examples ==========

  # Homepage featured products
  ProductOmQuery.search(%{featured: true, limit: 8})

  # Category page
  ProductOmQuery.search(%{
    category_slug: "electronics",
    sort: "bestselling",
    limit: 24
  })

  # Search results
  ProductOmQuery.search(%{
    q: "wireless headphones",
    min_price: 50,
    max_price: 300,
    brand: "Sony",
    in_stock: true,
    sort: "rating"
  })

  # Filter by multiple criteria
  ProductOmQuery.search(%{
    category_ids: [1, 2, 3],
    brand_ids: [10, 20],
    colors: ["black", "white"],
    sizes: ["M", "L", "XL"],
    min_rating: 4,
    on_sale: true,
    sort: "price_asc",
    limit: 48
  })

  # Pagination - next page
  ProductOmQuery.search(%{
    category_slug: "clothing",
    after: "g3QAAAACZAAKaW5zZXJ0ZWR..."
  })

  # Pagination - previous page
  ProductOmQuery.search(%{
    category_slug: "clothing",
    before: "g3QAAAACZAAKaW5zZXJ0ZWR..."
  })

  # Admin: low stock products
  ProductOmQuery.search(%{
    low_stock: true,
    sort: "name"
  })

  # Exclude certain statuses
  ProductOmQuery.search(%{
    exclude_statuses: ["draft", "archived", "discontinued"]
  })
  ```
  """
  def complete_real_world_example, do: :see_docs_above

  # ============================================================================
  # 17. PRODUCTION-READY COMPREHENSIVE SHOWCASE
  # ============================================================================
  #
  # This section demonstrates the Query module's production readiness through
  # a comprehensive example showcasing ALL available features:
  #
  # - Both DSL (macro) and Pipeline syntax side-by-side
  # - Cursor-based pagination at root, preload, and join levels
  # - All filter functions: comparison, range, string, null/blank, JSONB
  # - New features: between_any, composable filter builders, global opts
  # - Complex joins, CTEs, aggregations, window functions
  # - Conditional query building with then_if, if_true
  # - Search with ranking across multiple fields
  #
  # ============================================================================

  @doc """
  ## Production-Ready Assessment

  The OmQuery module is **production-ready** for complex queries because:

  ### 1. Complete Filter Coverage
  - All comparison operators: eq, neq, gt, gte, lt, lte
  - Range operators: in, not_in, between, between_any (list of ranges)
  - String patterns: like, ilike, not_like, not_ilike, starts_with, ends_with, contains_string
  - Null handling: is_nil, not_nil, where_nil, where_not_nil, where_blank, where_present
  - JSONB: jsonb_contains, jsonb_has_key, contains (array)
  - Subqueries: in_subquery, not_in_subquery
  - Similarity search: similarity, word_similarity, strict_word_similarity

  ### 2. Advanced Query Composition
  - OR/AND/NOT groups: where_any, where_all, where_none with global opts
  - Composable filter builders: condition/4, apply_any/3, apply_all/3, apply_none/3
  - Conditional building: then_if, if_true, scope, scopes
  - Fragment composition: include, include_if
  - Full-text search with ranking: search/4

  ### 3. Pagination at All Levels
  - Root level: cursor and offset pagination
  - Preload level: paginated associations
  - Join level: filtered and ordered joins
  - Automatic cursor field inference from order_by

  ### 4. Type-Safe Value Casting
  - Automatic casting: integer, float, decimal, boolean, date, datetime, uuid, atom
  - List casting for :in operators
  - Safe error handling for invalid values

  ### 5. Production Features
  - Query validation and error messages
  - Debug mode for SQL inspection
  - Streaming for large result sets
  - Batch parallel execution
  - Transaction support
  - Raw SQL escape hatch

  ---

  ## Comprehensive Example: E-Commerce Order Management System

  This example demonstrates a complex real-world query for an order management
  dashboard with:

  - **Root Query**: Orders with filters, search, and cursor pagination
  - **Joined Tables**: Customer, Products with bindings
  - **Nested Preloads**: Order items → Products → Category (3 levels deep)
  - **All Filter Types**: Every filter operator in use
  - **Multiple Pagination Levels**: Root + Preloads with cursors

  ---

  ## Pipeline Syntax - Order Management Dashboard

  Demonstrates ALL Query features using pipeline style.
  Best for: Dynamic queries, complex conditionals, maximum flexibility.
  """

  # ---------------------------------------------------------------------------
  # PIPELINE SYNTAX - Maximum Control & Flexibility
  # ---------------------------------------------------------------------------

  def order_dashboard_comprehensive_pipeline(params, current_user) do
    # Extract parameters
    search_term = params[:search]
    cursor = params[:after]
    status_list = params[:statuses] || ["pending", "processing", "shipped"]
    date_range = params[:date_range]
    # e.g., [{0, 100}, {500, 1000}, {2000, 5000}]
    price_tiers = params[:price_tiers]

    # =========================================================================
    # BUILD BASE QUERY WITH ALL FILTER TYPES
    # =========================================================================

    Order
    |> OmQuery.new()

    # -------------------------------------------------------------------------
    # 1. BASIC EQUALITY FILTERS
    # -------------------------------------------------------------------------
    |> OmQuery.filter(:deleted_at, :is_nil, true)
    |> OmQuery.filter(:organization_id, :eq, current_user.organization_id)

    # -------------------------------------------------------------------------
    # 2. LIST MEMBERSHIP FILTERS (in, not_in)
    # -------------------------------------------------------------------------
    |> OmQuery.filter(:status, :in, status_list)
    |> OmQuery.filter(:source, :not_in, ["test", "demo", "internal"])

    # -------------------------------------------------------------------------
    # 3. COMPARISON FILTERS (gt, gte, lt, lte)
    # -------------------------------------------------------------------------
    |> OmQuery.filter(:item_count, :gt, 0)
    # Convenience wrapper for :gte
    |> OmQuery.at_least(:subtotal, Decimal.new("10.00"))
    # Convenience wrapper for :lte
    |> OmQuery.at_most(:discount_percent, 50)

    # -------------------------------------------------------------------------
    # 4. RANGE FILTERS (between, between_any)
    # -------------------------------------------------------------------------
    |> OmQuery.then_if(date_range, fn token, {start_date, end_date} ->
      OmQuery.between(token, :created_at, start_date, end_date)
    end)
    # NEW: between_any for multiple ranges (OR of BETWEENs)
    |> OmQuery.then_if(price_tiers, fn token, tiers ->
      OmQuery.between_any(token, :total, tiers)
    end)

    # -------------------------------------------------------------------------
    # 5. STRING PATTERN FILTERS (like, ilike, starts_with, ends_with, contains)
    # -------------------------------------------------------------------------
    |> OmQuery.then_if(params[:order_prefix], fn token, prefix ->
      OmQuery.starts_with(token, :order_number, prefix)
    end)
    |> OmQuery.then_if(params[:email_domain], fn token, domain ->
      OmQuery.ends_with(token, :customer_email, "@#{domain}", case_insensitive: true)
    end)

    # -------------------------------------------------------------------------
    # 6. NULL/BLANK FILTERS (is_nil, not_nil, where_nil, where_not_nil)
    # -------------------------------------------------------------------------
    |> OmQuery.if_true(params[:shipped_only], fn token ->
      token
      |> OmQuery.where_not_nil(:shipped_at)
      |> OmQuery.where_present(:tracking_number)
    end)
    |> OmQuery.if_true(params[:unshipped], fn token ->
      token
      |> OmQuery.where_nil(:shipped_at)
      |> OmQuery.where_blank(:tracking_number)
    end)

    # -------------------------------------------------------------------------
    # 7. JSONB FILTERS (jsonb_contains, jsonb_has_key)
    # -------------------------------------------------------------------------
    |> OmQuery.then_if(params[:metadata_filter], fn token, meta ->
      OmQuery.filter(token, :metadata, :jsonb_contains, meta)
    end)
    |> OmQuery.if_true(params[:has_gift_message], fn token ->
      OmQuery.filter(token, :metadata, :jsonb_has_key, "gift_message")
    end)

    # -------------------------------------------------------------------------
    # 8. OR/AND/NOT GROUPS (where_any, where_all, where_none)
    # -------------------------------------------------------------------------
    # where_any with global opts (NEW: all filters use same binding)
    |> OmQuery.then_if(params[:priority_flags], fn token, _flags ->
      OmQuery.where_any(token, [
        {:priority, :eq, "high"},
        {:expedited, :eq, true},
        {:vip_customer, :eq, true}
      ])
    end)

    # where_all - require ALL conditions
    |> OmQuery.if_true(params[:complete_orders], fn token ->
      OmQuery.where_all(token, [
        {:payment_status, :eq, "paid"},
        {:shipped_at, :not_nil, true},
        {:delivered_at, :not_nil, true}
      ])
    end)

    # where_none - exclude matching NONE of these
    |> OmQuery.where_none([
      {:status, :eq, "cancelled"},
      {:status, :eq, "refunded"}
    ])

    # -------------------------------------------------------------------------
    # 9. COMPOSABLE FILTER BUILDERS (NEW)
    # -------------------------------------------------------------------------
    |> apply_dynamic_conditions(params)

    # -------------------------------------------------------------------------
    # 10. FULL-TEXT SEARCH WITH RANKING
    # -------------------------------------------------------------------------
    |> OmQuery.search(
      search_term,
      [
        {:order_number, :exact, rank: 1},
        {:customer_name, :ilike, rank: 2},
        {:customer_email, :ilike, rank: 3},
        {:notes, :similarity, threshold: 0.3, rank: 4}
      ],
      mode: :ilike,
      rank: true
    )

    # -------------------------------------------------------------------------
    # 11. JOINS WITH BINDINGS
    # -------------------------------------------------------------------------
    |> OmQuery.join(:customer, :left, as: :cust)
    |> OmQuery.join(:shipping_address, :left, as: :ship)
    |> OmQuery.join(:assigned_agent, :left, as: :agent)

    # Filter on joined tables using binding option
    |> OmQuery.then_if(params[:customer_tier], fn token, tier ->
      OmQuery.filter(token, :tier, :eq, tier, binding: :cust)
    end)
    |> OmQuery.then_if(params[:shipping_country], fn token, country ->
      OmQuery.filter(token, :country_code, :eq, country, binding: :ship)
    end)
    # where_any with global binding (NEW)
    |> OmQuery.then_if(params[:agent_filter], fn token, _filter ->
      OmQuery.where_any(
        token,
        [
          {:status, :eq, "available"},
          {:status, :eq, "assigned"}
        ],
        binding: :agent
      )
    end)

    # -------------------------------------------------------------------------
    # 12. SELECT WITH JOINED FIELDS
    # -------------------------------------------------------------------------
    |> OmQuery.select(%{
      id: :id,
      order_number: :order_number,
      status: :status,
      total: :total,
      created_at: :created_at,
      shipped_at: :shipped_at,
      # Fields from joined tables
      customer_id: {:cust, :id},
      customer_name: {:cust, :name},
      customer_email: {:cust, :email},
      customer_tier: {:cust, :tier},
      shipping_city: {:ship, :city},
      shipping_country: {:ship, :country_code},
      agent_name: {:agent, :name}
    })

    # -------------------------------------------------------------------------
    # 13. ORDERING WITH MULTIPLE FIELDS
    # -------------------------------------------------------------------------
    |> apply_order_sorting(params[:sort])

    # -------------------------------------------------------------------------
    # 14. CURSOR PAGINATION AT ROOT LEVEL
    # -------------------------------------------------------------------------
    |> OmQuery.then_if(cursor, fn token, c ->
      OmQuery.paginate(token, :cursor,
        limit: params[:limit] || 25,
        after: c
      )
    end)
    |> OmQuery.if_true(is_nil(cursor), fn token ->
      OmQuery.paginate(token, :cursor, limit: params[:limit] || 25)
    end)

    # -------------------------------------------------------------------------
    # 15. PRELOADS WITH PAGINATION AND FILTERING
    # -------------------------------------------------------------------------

    # Preload customer with their recent orders (cursor pagination in preload)
    |> OmQuery.preload(:customer, fn cust_token ->
      cust_token
      |> OmQuery.preload(:recent_orders, fn orders ->
        orders
        |> OmQuery.filter(:status, :in, ["completed", "shipped"])
        |> OmQuery.order(:created_at, :desc)
        |> OmQuery.order(:id, :asc)
        |> OmQuery.paginate(:cursor, limit: 5)
      end)
      |> OmQuery.preload(:addresses)
      |> OmQuery.select([:id, :name, :email, :tier, :lifetime_value])
    end)

    # Preload order items with products and categories (3 levels deep)
    |> OmQuery.preload(:order_items, fn items_token ->
      items_token
      |> OmQuery.filter(:deleted_at, :is_nil, true)
      |> OmQuery.order(:position, :asc)
      # Nested preload: Product → Category
      |> OmQuery.preload(:product, fn prod_token ->
        prod_token
        |> OmQuery.filter(:active, :eq, true)
        |> OmQuery.preload(:category, fn cat ->
          cat |> OmQuery.select([:id, :name, :slug])
        end)
        |> OmQuery.preload(:images, fn img ->
          img
          |> OmQuery.filter(:primary, :eq, true)
          |> OmQuery.limit(1)
        end)
        |> OmQuery.select([:id, :name, :sku, :price, :thumbnail_url])
      end)
    end)

    # Preload shipping events with cursor pagination
    |> OmQuery.preload(:shipping_events, fn events_token ->
      events_token
      |> OmQuery.filter(:deleted_at, :is_nil, true)
      |> OmQuery.order(:occurred_at, :desc)
      |> OmQuery.order(:id, :asc)
      |> OmQuery.paginate(:cursor, limit: 10)
      |> OmQuery.preload(:carrier)
    end)

    # Preload payment transactions
    |> OmQuery.preload(:transactions, fn tx_token ->
      tx_token
      |> OmQuery.filter(:status, :in, ["completed", "pending"])
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(5)
    end)

    # Preload notes (internal)
    |> OmQuery.preload(:notes, fn notes_token ->
      notes_token
      |> OmQuery.filter(:visibility, :eq, "internal")
      |> OmQuery.order(:created_at, :desc)
      |> OmQuery.limit(10)
      |> OmQuery.preload(:author, fn author ->
        author |> OmQuery.select([:id, :name, :avatar_url])
      end)
    end)

    # -------------------------------------------------------------------------
    # 16. EXECUTE WITH OPTIONS
    # -------------------------------------------------------------------------
    |> OmQuery.execute(include_total_count: true)
  end

  # Helper: Apply composable filter conditions
  defp apply_dynamic_conditions(token, params) do
    # Build conditions using composable filter builders
    conditions =
      []
      |> maybe_add_condition(params[:high_value], OmQuery.condition(:total, :gte, 1000))
      |> maybe_add_condition(params[:repeat_customer], OmQuery.condition(:is_repeat, :eq, true))
      |> maybe_add_condition(params[:has_discount], OmQuery.condition(:discount_amount, :gt, 0))
      |> maybe_add_condition(
        params[:express_shipping],
        OmQuery.condition(:shipping_method, :eq, "express")
      )

    case length(conditions) do
      0 ->
        token

      1 ->
        OmQuery.filter(
          token,
          elem(hd(conditions), 0),
          elem(hd(conditions), 1),
          elem(hd(conditions), 2)
        )

      # At least one must match
      _ ->
        OmQuery.apply_any(token, conditions)
    end
  end

  defp maybe_add_condition(conditions, true, condition), do: [condition | conditions]
  defp maybe_add_condition(conditions, _, _condition), do: conditions

  # Helper: Apply sorting based on params
  defp apply_order_sorting(token, "newest"), do: OmQuery.orders(token, desc: :created_at, asc: :id)
  defp apply_order_sorting(token, "oldest"), do: OmQuery.orders(token, asc: :created_at, asc: :id)
  defp apply_order_sorting(token, "total_high"), do: OmQuery.orders(token, desc: :total, asc: :id)
  defp apply_order_sorting(token, "total_low"), do: OmQuery.orders(token, asc: :total, asc: :id)

  defp apply_order_sorting(token, "priority"),
    do: OmQuery.orders(token, desc: :priority_score, asc: :created_at, asc: :id)

  defp apply_order_sorting(token, _), do: OmQuery.orders(token, desc: :created_at, asc: :id)

  # ---------------------------------------------------------------------------
  # DSL (MACRO) SYNTAX - Declarative & SQL-like
  #
  # Same functionality as pipeline version using macro DSL.
  # Best for: Static queries, SQL familiarity, readability.
  # ---------------------------------------------------------------------------

  def order_dashboard_comprehensive_dsl(params, current_user) do
    import OmQuery.DSL

    # Build base query with DSL - static parts
    base =
      query Order do
        # Equality filters
        filter(:deleted_at, :is_nil, true)
        filter(:organization_id, :eq, current_user.organization_id)

        # List membership
        filter(:status, :in, params[:statuses] || ["pending", "processing", "shipped"])
        filter(:source, :not_in, ["test", "demo", "internal"])

        # Comparison
        filter(:item_count, :gt, 0)

        # Exclude cancelled/refunded
        filters([
          {:status, :neq, "cancelled"},
          {:status, :neq, "refunded"}
        ])

        # Joins with bindings
        join(:customer, :left, as: :cust)
        join(:shipping_address, :left, as: :ship)
        join(:assigned_agent, :left, as: :agent)

        # Select with joined fields
        select(%{
          id: :id,
          order_number: :order_number,
          status: :status,
          total: :total,
          created_at: :created_at,
          shipped_at: :shipped_at,
          customer_id: {:cust, :id},
          customer_name: {:cust, :name},
          customer_email: {:cust, :email},
          customer_tier: {:cust, :tier},
          shipping_city: {:ship, :city},
          shipping_country: {:ship, :country_code},
          agent_name: {:agent, :name}
        })

        # Default ordering
        order(:created_at, :desc)
        order(:id, :asc)

        # Preload customer with nested associations
        preload :customer do
          preload :recent_orders do
            filter(:status, :in, ["completed", "shipped"])
            order(:created_at, :desc)
            order(:id, :asc)
            limit(5)
          end

          preload([:addresses])
          select([:id, :name, :email, :tier, :lifetime_value])
        end

        # Preload order items with products (3 levels deep)
        preload :order_items do
          filter(:deleted_at, :is_nil, true)
          order(:position, :asc)

          preload :product do
            filter(:active, :eq, true)

            preload :category do
              select([:id, :name, :slug])
            end

            preload :images do
              filter(:primary, :eq, true)
              limit(1)
            end

            select([:id, :name, :sku, :price, :thumbnail_url])
          end
        end

        # Preload shipping events with cursor pagination
        preload :shipping_events do
          filter(:deleted_at, :is_nil, true)
          order(:occurred_at, :desc)
          order(:id, :asc)
          paginate(:cursor, limit: 10)
          preload([:carrier])
        end

        # Preload transactions
        preload :transactions do
          filter(:status, :in, ["completed", "pending"])
          order(:created_at, :desc)
          limit(5)
        end

        # Preload notes with authors
        preload :notes do
          filter(:visibility, :eq, "internal")
          order(:created_at, :desc)
          limit(10)

          preload :author do
            select([:id, :name, :avatar_url])
          end
        end
      end

    # Apply conditional/dynamic parts using pipeline
    base
    # Search
    |> OmQuery.search(
      params[:search],
      [
        {:order_number, :exact, rank: 1},
        {:customer_name, :ilike, rank: 2},
        {:customer_email, :ilike, rank: 3},
        {:notes, :similarity, threshold: 0.3, rank: 4}
      ],
      mode: :ilike,
      rank: true
    )
    # Date range
    |> OmQuery.then_if(params[:date_range], fn token, {start_date, end_date} ->
      OmQuery.between(token, :created_at, start_date, end_date)
    end)
    # Multiple price ranges (NEW: between_any)
    |> OmQuery.then_if(params[:price_tiers], fn token, tiers ->
      OmQuery.between_any(token, :total, tiers)
    end)
    # String patterns
    |> OmQuery.then_if(params[:order_prefix], fn token, prefix ->
      OmQuery.starts_with(token, :order_number, prefix)
    end)
    |> OmQuery.then_if(params[:email_domain], fn token, domain ->
      OmQuery.ends_with(token, :customer_email, "@#{domain}", case_insensitive: true)
    end)
    # Null checks
    |> OmQuery.if_true(params[:shipped_only], fn token ->
      token
      |> OmQuery.where_not_nil(:shipped_at)
      |> OmQuery.where_present(:tracking_number)
    end)
    # JSONB filters
    |> OmQuery.then_if(params[:metadata_filter], fn token, meta ->
      OmQuery.filter(token, :metadata, :jsonb_contains, meta)
    end)
    # Priority flags (OR group with multiple conditions)
    |> OmQuery.then_if(params[:priority_flags], fn token, _flags ->
      OmQuery.where_any(token, [
        {:priority, :eq, "high"},
        {:expedited, :eq, true},
        {:vip_customer, :eq, true}
      ])
    end)
    # Customer tier filter on joined table
    |> OmQuery.then_if(params[:customer_tier], fn token, tier ->
      OmQuery.filter(token, :tier, :eq, tier, binding: :cust)
    end)
    # Shipping country filter on joined table
    |> OmQuery.then_if(params[:shipping_country], fn token, country ->
      OmQuery.filter(token, :country_code, :eq, country, binding: :ship)
    end)
    # Custom sorting
    |> apply_order_sorting(params[:sort])
    # Cursor pagination
    |> OmQuery.then_if(params[:after], fn token, cursor ->
      OmQuery.paginate(token, :cursor, limit: params[:limit] || 25, after: cursor)
    end)
    |> OmQuery.if_true(is_nil(params[:after]), fn token ->
      OmQuery.paginate(token, :cursor, limit: params[:limit] || 25)
    end)
    |> OmQuery.execute(include_total_count: true)
  end

  @doc """
  ### Usage Examples

  ```elixir
  # Basic usage - recent orders
  order_dashboard_comprehensive_pipeline(%{}, current_user)

  # With search and filters
  order_dashboard_comprehensive_pipeline(%{
    search: "ORD-2024",
    statuses: ["pending", "processing"],
    date_range: {~D[2024-01-01], ~D[2024-12-31]},
    shipped_only: true
  }, current_user)

  # With multiple price tiers (NEW: between_any)
  order_dashboard_comprehensive_pipeline(%{
    price_tiers: [{0, 50}, {200, 500}, {1000, 5000}],
    customer_tier: "gold",
    shipping_country: "US"
  }, current_user)

  # With priority filtering and sorting
  order_dashboard_comprehensive_pipeline(%{
    priority_flags: true,
    sort: "priority",
    limit: 50,
    after: "eyJpZCI6MTIzLCJjcmVhdGVkX2F0IjoiMjAyNC..."
  }, current_user)

  # With JSONB metadata filtering
  order_dashboard_comprehensive_pipeline(%{
    metadata_filter: %{"source" => "mobile", "campaign" => "summer_sale"},
    has_gift_message: true
  }, current_user)

  # DSL syntax produces identical results
  order_dashboard_comprehensive_dsl(%{
    search: "ORD-2024",
    statuses: ["pending"],
    date_range: {~D[2024-01-01], ~D[2024-06-30]}
  }, current_user)
  ```

  ### Result Structure

  ```elixir
  %OmQuery.Result{
    data: [
      %{
        id: 12345,
        order_number: "ORD-2024-00123",
        status: "processing",
        total: #Decimal<299.99>,
        created_at: ~U[2024-03-15 10:30:00Z],
        shipped_at: nil,
        customer_id: 789,
        customer_name: "John Doe",
        customer_email: "john@example.com",
        customer_tier: "gold",
        shipping_city: "San Francisco",
        shipping_country: "US",
        agent_name: "Support Agent",
        # Preloaded associations
        customer: %{
          id: 789,
          name: "John Doe",
          email: "john@example.com",
          tier: "gold",
          lifetime_value: #Decimal<5000.00>,
          recent_orders: [...],  # Cursor-paginated
          addresses: [...]
        },
        order_items: [
          %{
            position: 1,
            product: %{
              id: 456,
              name: "Widget Pro",
              sku: "WGT-PRO-001",
              price: #Decimal<99.99>,
              thumbnail_url: "https://...",
              category: %{id: 10, name: "Electronics", slug: "electronics"},
              images: [%{url: "https://...", primary: true}]
            }
          }
        ],
        shipping_events: [...],  # Cursor-paginated
        transactions: [...],
        notes: [...]
      }
    ],
    pagination: %{
      has_more: true,
      end_cursor: "eyJpZCI6MTIzNDUsImNyZWF0ZWRfYXQiOiIyMDI0Li4uIg==",
      start_cursor: "eyJpZCI6MTIzMDAsImNyZWF0ZWRfYXQiOiIyMDI0Li4uIg==",
      total_count: 1543
    },
    metadata: %{
      query_time_μs: 45230,
      source: Order,
      executed_at: ~U[2024-03-15 14:30:00Z]
    }
  }
  ```

  ### Production Checklist

  This example demonstrates production-ready patterns:

  - ✅ Tenant isolation (organization_id filter)
  - ✅ Soft delete handling (deleted_at checks)
  - ✅ Input validation (status whitelist)
  - ✅ Pagination at all levels (prevents memory issues)
  - ✅ Limited preloads (prevents N+1 and over-fetching)
  - ✅ Indexed fields for filtering (status, created_at, etc.)
  - ✅ Cursor-based pagination (stable for real-time data)
  - ✅ Total count for UI pagination controls
  - ✅ Query timing in metadata (for monitoring)
  """
  def showcase_documentation, do: :see_docs_above
end
