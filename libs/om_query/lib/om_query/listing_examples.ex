defmodule OmQuery.ListingExamples do
  ...
  def filters_chaining_pipeline do
    ...
    |> OmQuery.filter(:role, :in, ["admin", "editor"])
  end
  ...
  def product_listing(params) do
    base_query = OmQuery.new(Product)
    ...
    base_query
    |> OmQuery.filters(filters)
    |> OmQuery.orders(orders)
    |> OmQuery.paginate(:offset, limit: params[:per_page] || 20, offset: params[:offset] || 0)
  end
  ...
  def all_operators_showcase do
    ...
    separate =
      ...
      |> OmQuery.new()
      |> OmQuery.filter(:status, :eq, "active")
      |> OmQuery.filter(:category, :neq, "archived")
      |> OmQuery.filter(:price, :gt, 0)
      |> OmQuery.filter(:price, :gte, 10)
      |> OmQuery.filter(:stock, :lt, 100)
      |> OmQuery.filter(:stock, :lte, 50)
      |> OmQuery.filter(:category, :in, ["electronics", "gadgets"])
      |> OmQuery.filter(:tags, :not_in, ["discontinued"])
      |> OmQuery.filter(:name, :like, "%widget%")
      |> OmQuery.filter(:description, :ilike, "%smart%")
      |> OmQuery.filter(:deleted_at, :is_nil, nil)
      |> OmQuery.filter(:verified_at, :not_nil, nil)
      |> OmQuery.filter(:price, :between, {10.0, 100.0})
      |> OmQuery.filter(:features, :contains, ["wifi"])
      |> OmQuery.filter(:metadata, :jsonb_contains, %{featured: true})
      |> OmQuery.filter(:attributes, :jsonb_has_key, "color")
