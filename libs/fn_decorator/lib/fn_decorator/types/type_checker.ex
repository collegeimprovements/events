defmodule FnDecorator.Types.TypeChecker do
  @moduledoc """
  ...
  def handle_type_mismatch(message, true, context) do
    raise FnDecorator.Types.TypeError,
      message: "Type mismatch in #{FnDecorator.Support.Context.full_name(context)}: #{message}"
  end

  def handle_type_mismatch(message, false, context) do
    Logger.warning("Type mismatch in #{FnDecorator.Support.Context.full_name(context)}: #{message}")
    :ok
  end
end
