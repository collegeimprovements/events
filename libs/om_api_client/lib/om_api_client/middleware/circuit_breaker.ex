defmodule OmApiClient.Middleware.CircuitBreaker do
  @moduledoc """
  ...
  - `[:om_api_client, :circuit_breaker, :state_change]` - State transitions
  - `[:om_api_client, :circuit_breaker, :call]` - Call attempts
  """
  ...
  def init(opts) do
    state = %__MODULE__{
      ...
      should_trip: Keyword.get(opts, :should_trip)
    }
  ...
  def handle_call(:get_should_trip, _from, state) do
    {:reply, state.should_trip, state}
  end
  ...
  defp handle_result(name, {:error, error} = result) do
    if should_trip?(name, error) do
      record_failure(name)
    end
    result
  end

  defp should_trip?(name, error) do
    case GenServer.call(name, :get_should_trip) do
      nil -> true
  ...
  defp emit_telemetry(event, name, measurements) do
    :telemetry.execute(
      [:om_api_client, :circuit_breaker, event],
      measurements,
