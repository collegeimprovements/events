defmodule OmApiClient do
  @moduledoc """
  HTTP API client framework with middleware, retry, circuit breaker, and rate limiting.
  ...
  - `[:om_api_client, :request, :stop]` - Request completed
  - `[:om_api_client, :request, :exception]` - Request failed

  See `OmApiClient.Telemetry` for details.
  """

  alias OmApiClient.{Request, Response, Telemetry}
  ...
  defmacro __using__(opts) do
    ...
    response_module = Keyword.get(opts, :response_module, OmApiClient.Response)

    quote do
      @behaviour OmApiClient.Behaviour

      # Use the configured Request/Response modules
      alias unquote(request_module), as: Request
      alias unquote(response_module), as: Response
      ...
      if @telemetry_enabled do
        defp maybe_emit_telemetry_start(metadata) do
          Telemetry.emit_start(@client_name, metadata, @telemetry_prefix)
        end

        defp maybe_emit_telemetry_stop(nil, _metadata), do: :ok

        defp maybe_emit_telemetry_stop(start_time, metadata) do
          Telemetry.emit_stop(start_time, @client_name, metadata, @telemetry_prefix)
        end
        ...
        defp maybe_emit_telemetry_exception(start_time, kind, reason, stacktrace, metadata) do
          Telemetry.emit_exception(start_time, @client_name, kind, reason, stacktrace, metadata, @telemetry_prefix)
        end
      else
