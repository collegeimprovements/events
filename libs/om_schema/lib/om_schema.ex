defmodule OmSchema do
  ...
  Instead of `use Ecto.Schema`, use `OmSchema`:
  ...
  **All string fields are automatically trimmed by default!**
  ...
  defmacro schema(source, do: block) do
    quote do
      ...
      OmSchema.__generate_helpers__(__MODULE__)
    end
  end
  ...
  defmacro field(name, type \\ :string, opts \\ []) do
    quote bind_quoted: [name: name, type: type, opts: opts] do
      opts = OmSchema.merge_preset_opts(opts)
      {validation_opts, ecto_opts} = OmSchema.Field.__split_options__(opts, type, name)
      validation_opts = OmSchema.normalize_validation_opts(validation_opts)
  ...
  Delegates to `OmSchema.Constraints.constraints/1`.
  ...
  defmacro type_fields(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      for field <- [:type, :subtype], field in only do
        ...
        OmSchema.__define_field__(__MODULE__, field, :string, field_opts)
      end
    end
  end
  ...
  defmacro status_fields(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      ecto_opts =
        [values: values]
        |> OmSchema.maybe_put(:default, default)

      field_opts = Keyword.merge(ecto_opts, validation_opts)
      OmSchema.__define_enum_field__(__MODULE__, :status, values, field_opts)
    end
  end
  ...
  defmacro metadata_field(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      OmSchema.__define_field__(__MODULE__, :metadata, :map, field_opts)
    end
  end
  ...
  defmacro assets_field(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      default = Keyword.get(opts, :default, %{})
      field_opts = Keyword.put(opts, :default, default)
      OmSchema.__define_field__(__MODULE__, :assets, :map, field_opts)
    end
  end
  ...
  defmacro standard_fields(groups, opts) when is_list(groups) and is_list(opts) do
    quote bind_quoted: [groups: groups, opts: opts] do
      ...
      groups = OmSchema.resolve_groups(groups, except, all_groups)

      for group <- groups do
        group_opts = Keyword.get(opts, group, [])
        OmSchema.apply_field_group(__MODULE__, group, group_opts)
      end
    end
  end
  ...
  defmacro standard_fields(opts, []) when is_list(opts) and opts != [] do
    quote do
      OmSchema.standard_fields([], unquote(opts))
    end
  end
  ...
  ## Examples
      ...
      OmSchema.deletion_impact(account)
      # %{memberships: 5, roles: 3}

      # With depth - traverse association chain
      OmSchema.deletion_impact(account, depth: 2)
      # %{
      #   memberships: 5,
      #   roles: 3,
      ...
      def deletion_impact(record, opts \\\\ []) do
        OmSchema.deletion_impact(record, opts)
      end

  ## Options

    * `:depth` - How deep to traverse associations (default: 1)
    * `:repo` - Ecto repo to use (default: nil)
  ...
  def __define_enum_field__(module, name, values, opts) do
    {validation_opts, ecto_opts} = OmSchema.Field.__split_options__(opts, Ecto.Enum, name)
    validation_opts = normalize_validation_opts(validation_opts)
    ...
    quote do
      ...
      def apply_validations(changeset) do
        ...
        |> Enum.reduce(changeset, fn {field_name, field_type, opts}, acc ->
          OmSchema.Validation.apply_field_validation(acc, field_name, field_type, opts)
        end)
      end
      ...
      def validate_conditional_required(changeset, opts \\ []) do
        ...
        OmSchema.ConditionalRequired.validate(changeset, fields)
      end
      ...
      if Module.get_attribute(__MODULE__, :soft_delete_enabled) do
        ...
        def soft_delete_changeset(struct, opts \\ []) do
          ...
          changes =
            %{deleted_at: now}
            |> OmSchema.maybe_put(:deleted_by_urm_id, deleted_by)
