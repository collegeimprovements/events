defmodule OmSchema.Warnings do
  @moduledoc """
  Compile-time warning system for common OmSchema mistakes.
  ...
  defp check_required_when_syntax(opts, field_name) do
    case Keyword.get(opts, :required_when) do
      ...
      condition ->
        case OmSchema.ConditionalRequired.validate_syntax(condition) do
          :ok ->
            opts
    ...
    suggestions =
      cond do
        String.contains?(to_string(name), "email") && !opts[:format] ->
          ["Consider using OmSchema.Presets.email() for field #{name}" | suggestions]

        String.contains?(to_string(name), "url") && !opts[:format] ->
          ["Consider using OmSchema.Presets.url() for field #{name}" | suggestions]

        String.contains?(to_string(name), "slug") && !opts[:normalize] ->
          ["Consider using OmSchema.Presets.slug() for field #{name}" | suggestions]
