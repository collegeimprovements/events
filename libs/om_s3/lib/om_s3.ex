defmodule OmS3 do
  ...
  ## Pipeline Examples
      ...
      |> OmS3.method(:get)
      |> OmS3.presign_all(["s3://bucket/a.pdf", "s3://bucket/docs/*.pdf"])
  ...
  ## Batch Operations (Direct API)
      ...
      OmS3.get_all(["s3://bucket/*.txt"], config)
  ...
  Returns a Request struct for pipeline operations, or use `OmS3.config/0` for just config.
  ...
  ## Direct API
      ...
      :ok = OmS3.put("s3://bucket/file.txt", "hello", config)
  ...
  ## Pipeline API
      ...
      OmS3.new(config)
      |> OmS3.delete("s3://bucket/file.txt")
  """
  ...
  ## Pipeline API
      ...
      OmS3.new(config) |> OmS3.exists?("s3://bucket/file.txt")
  """
  ...
  ## Direct API
      ...
      {:ok, meta} = OmS3.head("s3://bucket/file.txt", config)

  ## Pipeline API

      OmS3.new(config) |> OmS3.head("s3://bucket/file.txt")
  """
  ...
  ## Direct API
      ...
      {:ok, %{files: files, next: nil}} = OmS3.list("s3://bucket/uploads/", config)

  ## Pipeline API

      OmS3.new(config)
      |> OmS3.list("s3://bucket/uploads/")
  ...
  ## Examples
      ...
      {:ok, all_files} = OmS3.list_all("s3://bucket/uploads/", config)
  """
  ...
  ## Pipeline API
      ...
      OmS3.new(config)
      |> OmS3.copy("s3://bucket/old.txt", "s3://bucket/new.txt")
  """
  ...
  ## Direct API
      ...
      {:ok, url} = OmS3.presign("s3://bucket/upload.jpg", config, method: :put)
  ...
  ## Examples
      ...
      {:ok, url} = OmS3.presign_get("s3://bucket/file.pdf", config, expires_in: {1, :hour})
  """
  ...
  ## Examples
      ...
      {:ok, url} = OmS3.presign_put("s3://bucket/upload.jpg", config)
  ...
  ## Direct API
      ...
      OmS3.put_all([{"s3://bucket/a.txt", "..."}], config)
      OmS3.put_all([{"a.txt", "..."}], config, to: "s3://bucket/")
  """
  ...
  ## Direct API
      ...
      OmS3.get_all(["s3://bucket/a.txt", "s3://bucket/*.json"], config)
  """
  ...
  ## Pipeline API
      ...
      OmS3.new(config)
      |> OmS3.copy_all([{"s3://src/a.txt", "s3://dst/a.txt"}])
  ...
  ## Direct API
      ...
      OmS3.copy_all([{"s3://src/a.txt", "s3://dst/a.txt"}], config)
      OmS3.copy_all("s3://source/*.jpg", config, to: "s3://dest/")
  """
  ...
  ## Pipeline API
      ...
      OmS3.new(config)
      |> OmS3.delete_all(["s3://bucket/temp/*.tmp"])

  ## Direct API

      OmS3.delete_all(["s3://bucket/old/*.txt"], config)
  """
  ...
  ## Pipeline API
      ...
      OmS3.new(config)
      |> OmS3.expires_in({1, :hour})
      |> OmS3.presign_all(["s3://bucket/*.pdf"])

  ## Direct API

      OmS3.presign_all(["s3://bucket/*.pdf"], config, expires_in: {5, :minutes})
  """
  ...
  ## Examples
      ...
      {:ok, "my-bucket", ""} = OmS3.parse_uri("s3://my-bucket")
