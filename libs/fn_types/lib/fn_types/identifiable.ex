defprotocol FnTypes.Protocols.Identifiable do
  ...
  ## Quick Start
      ...
      defmodule MyApp.User do
        @derive {FnTypes.Identifiable, type: :user}
        use Ecto.Schema
        # ...
      end
      ...
      FnTypes.Identifiable.identity(user)
      #=> {:user, "usr_123"}
  ...
  - `:id_field` - Field to use as ID (default: `:id`)
      ...
      # Custom ID field
      @derive {FnTypes.Identifiable, type: :invoice, id_field: :invoice_number}

      # Auto-derived type from module name (MyApp.User -> :user)
      @derive FnTypes.Identifiable

  ## Manual Implementation

  For non-schema types or custom behavior:

      defimpl FnTypes.Identifiable, for: MyApp.ExternalUser do
  ...
  Identifiable works well with other Events protocols:
      ...
      # With Cacheable
      defimpl MyApp.Cacheable, for: MyApp.User do
        def cache_key(user), do: FnTypes.Identifiable.identity(user)
        # ...
      end

      # With Loggable
      defimpl MyApp.Loggable, for: MyApp.User do
        def log_context(user) do
          {type, id} = FnTypes.Identifiable.identity(user)
  ...
  - `FnTypes.Error` - Uses error type and generated error ID
  - `Ecto.Changeset` - Extracts from underlying data or changes
  - Maps with `:id` key - Basic map support

  ## Telemetry

  Helper functions in `FnTypes.Identifiable.Helpers` emit telemetry:
