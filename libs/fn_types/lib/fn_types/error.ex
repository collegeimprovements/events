defmodule FnTypes.Error do
  ...
  ## Quick Start
      ...
      # Normalize any error
      FnTypes.Error.normalize({:error, :not_found})
      FnTypes.Error.normalize(%Ecto.Changeset{valid?: false})

      # Create a new error
      FnTypes.Error.new(:validation, :invalid_email,
      ...
      # Enrich with context
      error
      |> FnTypes.Error.with_context(user_id: 123, request_id: "req_123")

      # Store for analysis
      FnTypes.Error.store(error)

      # Handle in different contexts
      FnTypes.Error.handle(error, :http)  # Returns HTTP status and body
      FnTypes.Error.handle(error, :graphql)  # Returns GraphQL format
  ...
  # config :events, FnTypes.Error, task_supervisor: MyApp.TaskSupervisor
  @task_supervisor Application.compile_env(:fn_types, [__MODULE__, :task_supervisor], nil)
  ...
  This function delegates to the `FnTypes.Normalizable` protocol for type-based
  ...
  Implement `FnTypes.Normalizable` for custom error types:

      defimpl FnTypes.Normalizable, for: MyApp.CustomError do
        def normalize(error, opts) do
          FnTypes.Error.new(:business, error.code,
  ...
  defp get_configured_codes do
    Application.get_env(:fn_types, __MODULE__, [])
    |> Keyword.get(:codes, %{})
  end
