defmodule OmKillSwitch.Services.S3 do
  @moduledoc """
  ...
  ## Usage
      ...
      # Simple check
      if OmKillSwitch.Services.S3.enabled?() do
        OmS3.put("s3://bucket/key", content, config)
      end

      # With automatic fallback
      OmKillSwitch.Services.S3.upload("bucket", "key", content,
        config: my_config,
        fallback: fn -> DbStorage.save(key, content) end
      )

      # Pattern matching
      case OmKillSwitch.Services.S3.check() do
        :enabled -> OmS3.list("s3://bucket/", config)
        {:disabled, reason} -> {:error, {:s3_disabled, reason}}
      end

  ## Configuration

      # Disable S3 via environment
      S3_ENABLED=false
  ...
  - `:config` - S3 config (default: `OmS3.from_env()`)
  ...
  ## Examples
      ...
      # With fallback
      OmKillSwitch.Services.S3.list("my-bucket",
        prefix: "uploads/",
        fallback: fn -> {:ok, %{files: [], next: nil}} end
      )

      # Without fallback (returns error if disabled)
      OmKillSwitch.Services.S3.list("my-bucket", prefix: "uploads/")
  """
  ...
  def list(bucket, opts) when is_binary(bucket) do
    ...
    {config, s3_opts} = Keyword.pop(opts, :config)
    config = config || OmS3.from_env()
  ...
  ## Examples
      ...
      OmKillSwitch.Services.S3.upload("my-bucket", "photo.jpg", content,
  ...
  ## Examples
      ...
      OmKillSwitch.Services.S3.download("my-bucket", "photo.jpg",
  ...
  def download(bucket, path, opts) when is_binary(bucket) and is_binary(path) do
    ...
    config = Keyword.get(opts, :config) || OmS3.from_env()
    uri = OmS3.uri(bucket, path)
  ...
  ## Examples
      ...
      OmKillSwitch.Services.S3.delete("my-bucket", "old-file.txt",
  ...
  ## Examples
      ...
      OmKillSwitch.Services.S3.exists?("my-bucket", "photo.jpg",
  ...
  ## Examples
      ...
      OmKillSwitch.Services.S3.url_for_upload("my-bucket", "photo.jpg",
  ...
  ## Examples
      ...
      OmKillSwitch.Services.S3.url_for_download("my-bucket", "photo.jpg",
        expires_in: {1, :hour},
