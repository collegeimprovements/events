defmodule Events.Clients.Google.FCM do
  @moduledoc """
  ...
  ## Quick Start
      ...
      # Load credentials
      {:ok, creds} = FCM.credentials_from_env()

      # Send a simple notification
      {:ok, response} = FCM.push(creds,
  ...
  ## Sending Messages
      ...
      # To a topic
      FCM.push_to_topic(config, "news",
        title: "Breaking News",
        body: "Something happened!"
      )

      # To a condition (topic expression)
      FCM.push_to_condition(config, "'news' in topics && 'local' in topics",
        title: "Local News",
  ...
  ## Using with TokenServer (Recommended for Production)
      ...
      children = [
        {Events.Clients.Google.ServiceAccount.TokenServer,
          credentials: creds,
          scopes: Events.Clients.Google.FCM.scopes(),
          name: :fcm_token_server}
      ]

      # Then send messages using the token server
      FCM.push_with_server(:fcm_token_server, config,
        token: "device_token",
        title: "Hello",
        body: "World"
      )
  ...
  alias Events.Clients.Google.ServiceAccount
  ...
  ## Examples
      ...
      config = FCM.config(credentials: creds, project_id: "my-project")
  """
  ...
  ## Examples
      ...
      {:ok, config} = FCM.config_from_env()
      {:ok, config} = FCM.config_from_env("CUSTOM_FIREBASE_CREDENTIALS")
  """
  ...
  ## Examples
      ...
      FCM.push_to_condition(config, "'news' in topics || 'weather' in topics",
        title: "Update",
  ...
  ## Examples
      ...
      results = FCM.push_batch(config, messages)
