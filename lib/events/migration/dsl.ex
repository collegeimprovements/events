defmodule Events.Migration.DSL do
  @moduledoc """
  ...
  defmacro table(name, do: block) do
    quote do
      ...
      var!(current_token, Events.Migration.DSL) = token
      unquote(block)
      token = var!(current_token, Events.Migration.DSL)

      # Execute the migration
      Events.Migration.Executor.execute(token)
    end
  end
  ...
  defmacro field(name, type, opts \\ []) do
    quote do
      var!(current_token, Events.Migration.DSL) =
        Token.add_field(
          var!(current_token, Events.Migration.DSL),
          unquote(name),
          unquote(type),
  ...
  defmacro uuid_primary_key do
    quote do
      var!(current_token, Events.Migration.DSL) =
        Pipeline.with_uuid_primary_key(var!(current_token, Events.Migration.DSL))
    end
  end
  ...
  defmacro index(columns, opts \\ []) do
    quote do
      table_name = var!(current_token, Events.Migration.DSL).name
      index_name = Events.Migration.Helpers.index_name(table_name, unquote(columns))
  ...
  defmacro unique_index(columns) do
    quote do
      table_name = var!(current_token, Events.Migration.DSL).name
      index_name = Events.Migration.Helpers.unique_index_name(table_name, unquote(columns))
  ...
  defmacro belongs_to(name, table, opts \\ []) do
    quote do
      ...
      var!(current_token, Events.Migration.DSL) =
        Token.add_index(
          var!(current_token, Events.Migration.DSL),
          :"#{var!(current_token, Events.Migration.DSL).name}_#{field_name}_index",
          [field_name],
          []
        )
    end
