defmodule Events.Repo.Retry do
  @moduledoc """
  ...
  def transaction_with_retry(fun, opts \\ []) when is_function(fun, 0) do
    ...
    with_retry(
      fn ->
        Events.Repo.transaction(fun, transaction_opts)
      end,
      retry_opts
    )
  end
  ...
  def multi_with_retry(%Ecto.Multi{} = multi, opts \\ []) do
    ...
    with_retry(
      fn ->
        Events.Repo.transaction(multi, transaction_opts)
      end,
      retry_opts
    )
  end
  ...
  defp calculate_delay(error, attempt, base_delay, max_delay) do
    ...
    delay =
      if protocol_delay > 0 do
        ...
        Events.Recoverable.Backoff.exponential(attempt, base: base_delay, max: max_delay)
      end
