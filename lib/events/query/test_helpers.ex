defmodule Events.Query.TestHelpers do
  @moduledoc """
  Test utilities for the Events.Query system.
  ...
  Add to your test case:
      ...
      use Events.Query.TestHelpers

  Or import specific functions:

      import Events.Query.TestHelpers
  ...
  - `:repo` - The Ecto repo to use (default: Events.Repo)
  ...
  def to_sql_with_params(%Token{} = token, opts \\ []) do
    try do
      repo = opts[:repo] || Application.get_env(:events, :repo) || Events.Repo
  ...
  defmacro assert_has_operation(token, type) do
    quote do
      ...
      unless Events.Query.TestHelpers.has_operation?(token, type) do
        ops = Events.Query.TestHelpers.get_operations(token)
  ...
  defmacro refute_has_operation(token, type) do
    quote do
      ...
      if Events.Query.TestHelpers.has_operation?(token, type) do
        raise ExUnit.AssertionError,
  ...
  defmacro assert_filter_includes(token, field, op, value) do
    quote do
      ...
      filters = Events.Query.TestHelpers.get_operations_by_type(token, :filter)
  ...
  defmacro assert_sql_contains(token, substring) do
    quote do
      ...
      case Events.Query.TestHelpers.to_sql(token) do
        {:ok, sql} ->
  ...
  defmacro assert_pagination_type(token, expected_type) do
    quote do
      ...
      paginations = Events.Query.TestHelpers.get_operations_by_type(token, :paginate)
  ...
  defmacro assert_order_includes(token, field, direction) do
    quote do
      ...
      orders = Events.Query.TestHelpers.get_operations_by_type(token, :order)
  ...
  defmacro assert_join_includes(token, assoc, join_type \\ :inner) do
    quote do
      ...
      joins = Events.Query.TestHelpers.get_operations_by_type(token, :join)
  ...
  defmacro assert_operation_count(token, type, expected_count) do
    quote do
      ...
      ops = Events.Query.TestHelpers.get_operations_by_type(token, type)
  ...
  defmacro assert_tokens_equivalent(token1, token2) do
    quote do
      ...
      unless Events.Query.TestHelpers.tokens_equivalent?(t1, t2) do
        raise ExUnit.AssertionError,
          message: """
