defmodule Events.Maybe do
  ...
  ## Examples
      ...
      iex> Maybe.cat_maybes([{:some, 1}, :none, {:some, 3}])
      [1, 3]

      iex> Maybe.cat_maybes([:none, :none])
      []
  """
  @spec cat_maybes([t(a)]) :: [a] when a: term()
  def cat_maybes(maybes) when is_list(maybes) do
    maybes
    |> Enum.filter(&some?/1)
    |> Enum.map(&unwrap!/1)
  end
