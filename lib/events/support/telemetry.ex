defmodule Events.Support.Telemetry do
  ...
  ## Usage
      ...
      use Events.Support.Telemetry

      # Automatic span tracking
      span [:events, :crud, :create], %{schema: User} do
        Repo.insert(changeset)
      end

      # Manual emission
      emit [:events, :cache, :hit], %{key: key}, %{size: byte_size(value)}

  ## Event Naming Convention

  All events follow the pattern: `[:events, :domain, :operation, :phase]`
  ...
  defmacro __using__(_opts) do
    quote do
      import Events.Support.Telemetry, only: [span: 3, span: 4, emit: 3, emit: 4, timed: 2]
      require Events.Support.Telemetry
    end
  end
  ...
  ## Examples
      ...
      # With options
      span [:events, :api, :call], %{endpoint: url}, include_result: true do
        http_client.get(url)
      end
  ...
  defmacro span(_event, _metadata, opts) when is_list(opts) do
    # This clause is a placeholder that matches span(event, meta, opts)
    # The actual implementation is in span/4 which handles the do: block
    quote do
      unquote(opts)
    end
  end
  ...
  ## Examples
      ...
      span [:events, :api, :call], %{url: url}, [include_result: true] do
        make_request(url)
      end
  """
  ...
  ## Examples
      ...
      emit [:events, :cache, :hit], %{}, %{key: key}
      emit [:events, :queue, :enqueue], %{queue_size: 42}, %{job: job_name}
  """
  ...
  ## Examples
      ...
      emit_with_time [:events, :user, :login], %{}, %{user_id: id}
  """
  ...
  ## Examples
      ...
      # With options
      {time_ms, result} = timed(fn -> query() end, unit: :microsecond)
  """
  ...
  ## Examples
      ...
      ctx = start_span([:events, :batch, :process], %{batch_id: id})
      # ... do work ...
      stop_span(ctx)
  """
  ...
  ## Examples
      ...
      Events.Support.Telemetry.attach_logger([:events, :crud])

      # With custom log level
      Events.Support.Telemetry.attach_logger([:events, :api], level: :debug)
  """
