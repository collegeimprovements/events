defmodule Events.Core.Schema.Field do
  @moduledoc """
  ...
  @doc """
  Backwards-compatible `field/3` macro that now delegates to `Events.Core.Schema.field/3`.

  Prefer `use Events.Core.Schema` and its imported macro directly. This definition
  remains so older modules that `import Events.Core.Schema.Field` continue to work,
  but the implementation lives in `Events.Core.Schema` to avoid divergence.
  """
  @deprecated "Import Events.Core.Schema.field/3 via `use Events.Core.Schema` instead"
  defmacro field(name, type \\ :string, opts \\ []) do
    quote do
      require Events.Core.Schema
      Events.Core.Schema.field(unquote(name), unquote(type), unquote(opts))
    end
  end
