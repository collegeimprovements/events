defmodule Events.Core.Schema.Warnings do
  @moduledoc """
  Compile-time warning system for common Events.Core.Schema mistakes.
  ...
  defp check_required_when_syntax(opts, field_name) do
    case Keyword.get(opts, :required_when) do
      ...
      condition ->
        case Events.Core.Schema.ConditionalRequired.validate_syntax(condition) do
          :ok ->
            opts
    ...
    suggestions =
      cond do
        String.contains?(to_string(name), "email") && !opts[:format] ->
          ["Consider using Events.Core.Schema.Presets.email() for field #{name}" | suggestions]

        String.contains?(to_string(name), "url") && !opts[:format] ->
          ["Consider using Events.Core.Schema.Presets.url() for field #{name}" | suggestions]

        String.contains?(to_string(name), "slug") && !opts[:normalize] ->
          ["Consider using Events.Core.Schema.Presets.slug() for field #{name}" | suggestions]
