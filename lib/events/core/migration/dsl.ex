defmodule Events.Core.Migration.DSL do
  @moduledoc """
  ...
  defmacro table(name, do: block) do
    quote do
      ...
      var!(current_token, Events.Core.Migration.DSL) = token
      unquote(block)
      token = var!(current_token, Events.Core.Migration.DSL)

      # Execute the migration
      Events.Core.Migration.Executor.execute(token)
    end
  end
  ...
  defmacro uuid_primary_key do
    quote do
      var!(current_token, Events.Core.Migration.DSL) =
        Pipeline.with_uuid_primary_key(var!(current_token, Events.Core.Migration.DSL))
    end
  end
  ...
  defmacro index(columns, opts \\ []) do
    quote do
      table_name = var!(current_token, Events.Core.Migration.DSL).name
      index_name = Events.Core.Migration.Helpers.index_name(table_name, unquote(columns))
  ...
  defmacro unique_index(columns) do
    quote do
      table_name = var!(current_token, Events.Core.Migration.DSL).name
      index_name = Events.Core.Migration.Helpers.unique_index_name(table_name, unquote(columns))
  ...
  defmacro belongs_to(name, table, opts \\ []) do
    quote do
      ...
      var!(current_token, Events.Core.Migration.DSL) =
        Token.add_index(
          var!(current_token, Events.Core.Migration.DSL),
          :"#{var!(current_token, Events.Core.Migration.DSL).name}_#{field_name}_index",
          [field_name],
          []
        )
    end
