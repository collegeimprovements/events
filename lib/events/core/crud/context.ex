defmodule Events.Core.Crud.Context do
  ...
  ## Examples
      ...
      defmodule MyApp.Accounts do
        ...
        crud Membership, as: :member, preload: [:user, :account]
      end

  ## Crud-level Options

  You can configure default options that apply to all generated functions for a schema:

      defmodule MyApp.Accounts do
        use Events.Core.Crud.Context

        # Use a read replica for all User reads with higher timeout
        crud User, repo: MyApp.ReadOnlyRepo, timeout: 30_000

        # Multi-tenant setup with logging disabled for Role
  ...
  These defaults can still be overridden at call time:

      # Uses MyApp.ReadOnlyRepo by default
      ...
      Accounts.fetch_role(id)
      Accounts.create_role(%{name: "admin"})
      Accounts.list_roles()

      Accounts.fetch_member(id, preload: [:user])
      Accounts.create_member(%{user_id: uid, account_id: aid})
  """
  ...
  defmacro __using__(_opts) do
    quote do
      import Events.Core.Crud.Context, only: [crud: 1, crud: 2]
  ...
  defp generate_function(
    ...
    quote do
      ...
      def unquote(fn_name)(id, opts \\ []) do
        ...
        Events.Core.Crud.fetch(unquote(schema), id, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(
    ...
    quote do
      ...
      def unquote(fn_name)(id, opts \\ []) do
        ...
        Events.Core.Crud.get(unquote(schema), id, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(
    ...
    quote do
      ...
      def unquote(fn_name)(opts \\ []) do
        ...
        Events.Core.Crud.fetch_all(unquote(schema), opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(:exists?, schema, resource, _resources, _preload, _changeset, default_opts) do
    ...
    quote do
      ...
      def unquote(fn_name)(id, opts \\ []) do
        ...
        Events.Core.Crud.exists?(unquote(schema), id, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(:create, schema, resource, _resources, _preload, changeset, default_opts) do
    ...
    quote do
      ...
      def unquote(fn_name)(attrs, opts \\ []) do
        ...
        Events.Core.Crud.create(unquote(schema), attrs, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(:update, _schema, resource, _resources, _preload, changeset, default_opts) do
    ...
    quote do
      ...
      def unquote(fn_name)(record, attrs, opts \\ []) do
        ...
        Events.Core.Crud.update(record, attrs, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(:delete, _schema, resource, _resources, _preload, _changeset, default_opts) do
    ...
    quote do
      ...
      def unquote(fn_name)(record, opts \\ []) do
        opts = Keyword.merge(unquote(default_opts), opts)
        Events.Core.Crud.delete(record, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(
    ...
    quote do
      ...
      def unquote(fn_name)(entries, opts \\ []) do
        opts = Keyword.merge(unquote(default_opts), opts)
        Events.Core.Crud.create_all(unquote(schema), entries, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(
    ...
    quote do
      ...
      def unquote(fn_name)(query \\ unquote(schema), updates, opts \\ []) do
        opts = Keyword.merge(unquote(default_opts), opts)
        Events.Core.Crud.update_all(query, updates, opts)
      end
    end
    |> List.wrap()
  end
  ...
  defp generate_function(
    ...
    quote do
      ...
      def unquote(fn_name)(query \\ unquote(schema), opts \\ []) do
        opts = Keyword.merge(unquote(default_opts), opts)
        Events.Core.Crud.delete_all(query, opts)
      end
    end
    |> List.wrap()
  end
