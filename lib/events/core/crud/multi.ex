defmodule Events.Core.Crud.Multi do
  ...
  ## Usage
      ...
      # Execute explicitly
      {:ok, %{user: user, account: account}} = Crud.run(multi)
...
defimpl Events.Core.Crud.Executable, for: Events.Core.Crud.Multi do
  alias Events.Core.Crud.Multi

  def execute(%Multi{} = multi, opts) do
    Events.Core.Crud.transaction(multi, opts)
  end
end

defimpl Events.Core.Crud.Validatable, for: Events.Core.Crud.Multi do
  alias Events.Core.Crud.Multi
...
defimpl Events.Core.Crud.Debuggable, for: Events.Core.Crud.Multi do
  alias Events.Core.Crud.Multi
...
defimpl Inspect, for: Events.Core.Crud.Multi do
  alias Events.Core.Crud.Multi
  ...
  def inspect(%Multi{} = multi, _opts) do
    ...
    if length(names) <= 5 do
      "#Crud.Multi<#{Enum.join(names, ", ")}>"
    else
      shown = Enum.take(names, 3)
      "#Crud.Multi<#{Enum.join(shown, ", ")}, ... (#{length(names)} total)>"
    end
  end
end
