defmodule Events.Core.Crud.Merge do
  ...
  """
  @spec to_sql(t()) :: {String.t(), [term()]}
  def to_sql(%__MODULE__{} = merge) do
    # This is a placeholder - actual SQL generation would be complex
    # and depend on the specific PostgreSQL 18 MERGE syntax
    {build_merge_sql(merge), build_params(merge)}
  end

  defp build_merge_sql(%{schema: schema, match_on: match_on} = merge) do
    table = schema.__schema__(:source)

    """
    MERGE INTO #{table} AS target
    USING #{source_sql(merge)} AS source
  ...
  defp source_sql(%{source: source}) when is_map(source) do
    "(VALUES (#{placeholder_values(source)})) AS source(#{column_names(source)})"
  end

  defp source_sql(%{source: sources}) when is_list(sources) do
    values =
      sources
  ...
  defp source_sql(%{source: query}) when is_struct(query) do
    {sql, _params} = Events.Core.Repo.to_sql(:all, query)
    "(#{sql})"
  end
...
defimpl Events.Core.Crud.Executable, for: Events.Core.Crud.Merge do
  alias Events.Core.Crud.Merge

  def execute(%Merge{} = merge, opts) do
    # Delegate to the main Crud module for execution
    Events.Core.Crud.execute_merge(merge, opts)
  end
end

defimpl Events.Core.Crud.Validatable, for: Events.Core.Crud.Merge do
  alias Events.Core.Crud.Merge
...
defimpl Events.Core.Crud.Debuggable, for: Events.Core.Crud.Merge do
  alias Events.Core.Crud.Merge
...
defimpl Inspect, for: Events.Core.Crud.Merge do
  alias Events.Core.Crud.Merge
  ...
  def inspect(%Merge{} = merge, _opts) do
    ...
    "#Crud.Merge<#{schema_name}, match: #{inspect(match_on)}, source: #{source_info}>"
  end
end
