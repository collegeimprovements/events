defprotocol Events.Core.Crud.Executable do
  ...
  for consistent behavior and unified execution via `Crud.run/1`.

  ## Implementations

  - `Events.Core.Crud.Multi` - Transaction composition
  - `Events.Core.Crud.Merge` - PostgreSQL MERGE operations
  - `Events.Core.Query.Token` - Query execution (fetch/fetch_all)
