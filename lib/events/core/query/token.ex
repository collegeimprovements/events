defmodule Events.Core.Query.Token do
  @moduledoc false
  ...
  alias __MODULE__
  alias Events.Core.Query.{ValidationError, LimitExceededError, PaginationError}
  ...
  def add_operation_safe(%Token{} = token, operation) when is_tuple(operation) do
    case validate_operation(operation) do
      ...
      {:error, %Events.Core.Query.FilterGroupError{} = error} ->
        {:error, error}
  ...
  defp validate_filter_group(combinator, filters) when length(filters) < 2 do
    {:error,
     %Events.Core.Query.FilterGroupError{
