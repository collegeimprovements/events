defmodule Events.Core.Query.Helpers do
  @moduledoc """
  ...
  """
  @spec dynamic_filters(Events.Core.Query.Token.t(), map(), map()) :: Events.Core.Query.Token.t()
  def dynamic_filters(token, filters, mapping) when is_map(filters) and is_map(mapping) do
    Enum.reduce(mapping, token, fn {param_key, {op, field}}, acc ->
      case Map.get(filters, param_key) do
        nil -> acc
        value -> Events.Core.Query.where(acc, field, op, value)
      end
    end)
  end
  ...
  """
  @spec ensure_limit(Events.Core.Query.Token.t(), pos_integer()) :: Events.Core.Query.Token.t()
  def ensure_limit(token, default_limit) do
    ...
    if has_limit do
      ...
      Events.Core.Query.limit(token, default_limit)
    end
  end
  ...
  """
  @spec sort_by(Events.Core.Query.Token.t(), String.t() | nil) :: Events.Core.Query.Token.t()
  ...
  def sort_by(token, sort_string) when is_binary(sort_string) do
    ...
    |> Enum.reduce(token, fn field_spec, acc ->
      case field_spec do
        "-" <> field ->
          Events.Core.Query.order_by(acc, String.to_existing_atom(field), :desc)

        "+" <> field ->
          Events.Core.Query.order_by(acc, String.to_existing_atom(field), :asc)
  ...
  """
  @spec safe_sort_by(Events.Core.Query.Token.t(), String.t() | nil) ::
          {:ok, Events.Core.Query.Token.t()} | {:error, :invalid_field}
  ...
  """
  @spec paginate_from_params(Events.Core.Query.Token.t(), map()) :: Events.Core.Query.Token.t()
  def paginate_from_params(token, params) when is_map(params) do
    limit = parse_int(params["limit"], 20)

    cond do
      cursor = params["cursor"] ->
        Events.Core.Query.paginate(token, :cursor, limit: limit, after: cursor)

      offset = params["offset"] ->
        offset_int = parse_int(offset, 0)
        Events.Core.Query.paginate(token, :offset, limit: limit, offset: offset_int)

      true ->
        Events.Core.Query.paginate(token, :cursor, limit: limit)
    end
  end
