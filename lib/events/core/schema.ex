defmodule Events.Core.Schema do
  ...
  Instead of `use Ecto.Schema`, use `Events.Core.Schema`:
  ...
  **All string fields are automatically trimmed by default!**
  ...
  defmacro schema(source, do: block) do
    quote do
      ...
      Events.Core.Schema.__generate_helpers__(__MODULE__)
    end
  end
  ...
  Delegates to `Events.Core.Schema.Constraints.constraints/1`.
  ...
  defmacro constraints(do: block) do
    quote do
      import Events.Core.Schema.Constraints,
  ...
  defmacro type_fields(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      for field <- [:type, :subtype], field in only do
        ...
        Events.Core.Schema.__define_field__(__MODULE__, field, :string, field_opts)
      end
    end
  end
  ...
  defmacro status_fields(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      ecto_opts =
        [values: values]
        |> Events.Core.Schema.maybe_put(:default, default)

      field_opts = Keyword.merge(ecto_opts, validation_opts)
      Events.Core.Schema.__define_enum_field__(__MODULE__, :status, values, field_opts)
    end
  end
  ...
  defmacro audit_fields(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      for field <- [:created_by_urm_id, :updated_by_urm_id], field in only do
        field_opts = Keyword.get(opts, field, [])
        Events.Core.Schema.__define_field__(__MODULE__, field, :binary_id, field_opts)
      end
    end
  end
  ...
  defmacro metadata_field(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      Events.Core.Schema.__define_field__(__MODULE__, :metadata, :map, field_opts)
    end
  end
  ...
  defmacro assets_field(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      default = Keyword.get(opts, :default, %{})
      field_opts = Keyword.put(opts, :default, default)
      Events.Core.Schema.__define_field__(__MODULE__, :assets, :map, field_opts)
    end
  end
  ...
  defmacro soft_delete_field(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      Events.Core.Schema.__define_field__(
        __MODULE__,
        :deleted_at,
        :utc_datetime_usec,
        deleted_at_opts
  ...
  defmacro standard_fields(groups, opts) when is_list(groups) and is_list(opts) do
    quote bind_quoted: [groups: groups, opts: opts] do
      ...
      groups = Events.Core.Schema.resolve_groups(groups, except, all_groups)

      for group <- groups do
        group_opts = Keyword.get(opts, group, [])
        Events.Core.Schema.apply_field_group(__MODULE__, group, group_opts)
      end
    end
  end
  ...
  defmacro standard_fields(opts, []) when is_list(opts) and opts != [] do
    quote do
      Events.Core.Schema.standard_fields([], unquote(opts))
    end
  end
  ...
  ## Examples
      ...
      Events.Core.Schema.deletion_impact(account)
      # %{memberships: 5, roles: 3}

      # With depth - traverse association chain
      Events.Core.Schema.deletion_impact(account, depth: 2)
      # %{
      #   memberships: 5,
      #   roles: 3,
      ...
      def deletion_impact(record, opts \\\\ []) do
        Events.Core.Schema.deletion_impact(record, opts)
      end
  ...
  ## Examples
      ...
      impact = Events.Core.Schema.deletion_impact(account, depth: 2)
      Events.Core.Schema.format_deletion_impact(impact)
    ...
    quote do
      ...
      def apply_validations(changeset) do
        ...
        |> Enum.reduce(changeset, fn {field_name, field_type, opts}, acc ->
          Events.Core.Schema.Validation.apply_field_validation(acc, field_name, field_type, opts)
        end)
      end
      ...
      def validate_conditional_required(changeset, opts \\ []) do
        ...
        Events.Core.Schema.ConditionalRequired.validate(changeset, fields)
      end
      ...
      if Module.get_attribute(__MODULE__, :soft_delete_enabled) do
        ...
        def soft_delete_changeset(struct, opts \\ []) do
          ...
          changes =
            %{deleted_at: now}
            |> Events.Core.Schema.maybe_put(:deleted_by_urm_id, deleted_by)
      ...
      @field_unique_constraints @field_validations
                                ...
                                |> Enum.map(fn {name, _type, opts} ->
                                  Events.Core.Schema.Constraints.normalize_unique_option(
                                    opts[:unique],
      ...
      @field_check_constraints @field_validations
                               ...
                               |> Enum.map(fn {name, _type, opts} ->
                                 Events.Core.Schema.Constraints.normalize_check_option(
                                   opts[:check],
                                   @__schema_table_name__,
                                   name
                                 )
      ...
      @belongs_to_fk_constraints (Module.get_attribute(__MODULE__, :belongs_to_fields) || [])
                                 |> Enum.map(fn {_name, fk_field, queryable, constraint_config} ->
                                   ...
                                   Events.Core.Schema.Constraints.normalize_belongs_to_constraint(
                                     constraint_config,
                                     @__schema_table_name__,
                                     fk_field,
                                     related_table
