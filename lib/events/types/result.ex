defmodule Events.Types.Result do
  @moduledoc """
  ...
  ## Examples
      ...
      #=> {:error, %Events.Types.Error{type: :validation, ...}}
      ...
      #=> {:error, %Events.Types.Error{context: %{user_id: 123}, ...}}

      {:ok, value} |> Result.normalize_error()
      #=> {:ok, value}
  """
  @spec normalize_error(t(v, e), keyword()) :: t(v, Events.Types.Error.t())
        when v: term(), e: term()
  ...
  @doc """
  Wraps an error with Events.Types.Error context.
  ...
  @doc """
  Converts an error to an Events.Types.Error struct.
  ...
  ## Examples
      ...
      #=> {:error, %Events.Types.Error{type: :not_found, code: :not_found, ...}}

      Result.to_error({:ok, value}, :not_found)
      #=> {:ok, value}
  """
  @spec to_error(t(v, e), atom(), keyword()) :: t(v, Events.Types.Error.t())
        when v: term(), e: term()
  ...
  def to_error({:error, reason}, type, opts) when is_atom(type) do
    ...
    error = Events.Types.Error.new(type, code, message: message, details: details, context: context)
    {:error, error}
  end
