# Implementation for Mint.TransportError and Mint.HTTPError
# These are low-level HTTP transport errors
# NOTE: Defined in Events because Mint is a transitive dependency here,
# and fn_types compiles before Mint is available.

defimpl FnTypes.Protocols.Normalizable, for: Mint.TransportError do
  @moduledoc """
  ...
  defp map_reason(:nxdomain),
    do: {:network, :dns_error, "DNS resolution failed - domain does not exist", true}
  ...
  defp map_reason({:tls_alert, {:certificate_expired, _}}),
    do: {:network, :certificate_expired, "TLS certificate expired", true}

  defp map_reason({:tls_alert, {:certificate_revoked, _}}),
    do: {:network, :certificate_revoked, "TLS certificate revoked", true}

  defp map_reason({:tls_alert, {:unknown_ca, _}}),
    do: {:network, :unknown_ca, "Unknown certificate authority", true}
  ...
  defp map_reason({:tls_alert, {alert, _}}),
    do: {:network, :tls_error, "TLS error: #{alert}", true}
...
defimpl FnTypes.Protocols.Normalizable, for: Mint.HTTPError do
  @moduledoc """
