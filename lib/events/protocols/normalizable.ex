defprotocol Events.Protocols.Normalizable do
  @fallback_to_any true

  @moduledoc """
  Backwards-compatibility alias for `Events.Types.Normalizable`.

  > **Deprecated**: Use `Events.Types.Normalizable` instead.
  > This module exists for backwards compatibility and delegates to the new location.

  See `Events.Types.Normalizable` for full documentation.
  """

  @doc """
  Normalize the error into a standard `Events.Types.Error` struct.

  **Deprecated**: Use `Events.Types.Normalizable.normalize/2` instead.
  """
  @spec normalize(t(), keyword()) :: Events.Types.Error.t()
  def normalize(error, opts \\ [])
end

# Note: Protocol implementations remain in lib/events/protocols/impls/normalizable/
# They implement Events.Protocols.Normalizable which is still the canonical protocol
# used throughout the codebase during the transition period.
