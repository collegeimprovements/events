defmodule Events.Credo.Checks.UseEventsSchema do
  @moduledoc """
  Checks that all schemas in the lib/ directory use `Events.Schema` instead of raw `Ecto.Schema`.

  This ensures consistency across the codebase and leverages our enhanced schema features.

  ## Why This Matters

  `Events.Schema` provides:
  - UUIDv7 primary keys
  ...
  Incorrect:
      ...
      defmodule MyApp.User do
        use Ecto.Schema  # Bad - use Events.Schema instead
      end
  ...
  use Credo.Check,
    ...
    explanations: [
      check: """
      Always use `Events.Schema` instead of `Ecto.Schema` in lib/ files.

      Events.Schema wraps Ecto.Schema and provides additional features like
  ...
  defp issue_for(issue_meta, line_no, trigger) do
    format_issue(
      issue_meta,
      message: "Use `Events.Schema` instead of `Ecto.Schema` for consistency.",
      trigger: trigger,
      line_no: line_no
    )
  end
