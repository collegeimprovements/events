  ...
  def run(args) do
    ...
    result =
      case args do
        [] ->
          # Validate all schemas
          Events.Schema.DatabaseValidator.validate_all(validation_opts)

        [pattern] ->
          if String.contains?(pattern, "*") do
            # Pattern match
            schemas = Events.Schema.DatabaseValidator.discover_schemas(pattern)
            validate_multiple(schemas, validation_opts)
          else
            # Single schema
            schema_module = Module.concat([pattern])

            Events.Schema.DatabaseValidator.validate(schema_module, validation_opts)
  ...
  defp validate_multiple(schemas, opts) do
    results =
      Enum.map(schemas, fn schema ->
        {schema, Events.Schema.DatabaseValidator.validate(schema, opts)}
      end)
